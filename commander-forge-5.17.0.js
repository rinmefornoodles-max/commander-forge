...
const MULTIPLAYER_APP_VERSION = '5.17.0';
const multiplayer = {
  mode: 'solo',
  role: null,
  localPlayerId: null,
...
function attachMultiplayerConnection(connection) {
  ...
  connection.on('error', (error) => {
    ...
    if (/too big for JSON channel/i.test(message)) {
      multiplayer.lastError = 'The other browser is still using the old JSON multiplayer channel. Both players must refresh to version 5.16.';
