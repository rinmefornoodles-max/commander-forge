...
function renderInspector(state) {
  ...
  return `
    ...
    ${mutateInfo.hasMutate && mutateInfo.canCastMutate ? `<div class="inspector-section"><h3>Mutate ${escapeHtml(mutateInfo.cost)}${mutateInfo.commanderTax ? ` + {${mutateInfo.commanderTax}} tax` : ''}</h3><p class="small muted">Cast this creature for its Mutate alternative cost onto a non-Human creature you own.${selected.zone === 'command' ? ' Because this is being cast from the command zone, commander tax is added to the Mutate cost.' : ''} Choose whether the mutating card goes on top of or underneath the target.</p>${mutateActions.length ? mutateActions.map((item) => `<div class="mana-choice-group"><div class="small muted wide">Target: ${escapeHtml(item.targetName)} · ${escapeHtml(item.cost)}</div><button class="btn primary" data-action="cast-mutate" data-card-id="${card.instanceId}" data-target-id="${item.targetId}" data-position="top">Mutate on top</button><button class="btn" data-action="cast-mutate" data-card-id="${card.instanceId}" data-target-id="${item.targetId}" data-position="under">Mutate underneath</button>${state.settings.testOverrides ? `<button class="btn ghost" data-action="cast-mutate" data-card-id="${card.instanceId}" data-target-id="${item.targetId}" data-position="top" data-test-override="true">Test mutate on top · no mana</button><button class="btn ghost" data-action="cast-mutate" data-card-id="${card.instanceId}" data-target-id="${item.targetId}" data-position="under" data-test-override="true">Test mutate underneath · no mana</button>` : ''}</div>`).join('') : `<div class="validation warn"><strong>Mutate is currently unavailable.</strong><br />${escapeHtml(mutateInfo.reason || 'No legal mutate target is available.')}</div>`}</div>` : ''}
    ${renderMutateRules(card, selected, state, mutateInfo)}
