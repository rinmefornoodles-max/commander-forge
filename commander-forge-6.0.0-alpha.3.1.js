...
function botModalBlocked() { return Boolean(ui.setupOpen || ui.settingsOpen || ui.scry || ui.localDialog || ui.undoRequest || ui.overrideRequest || ui.loading); }
...
function renderSettingsModal(state) {
  const v6 = window.CommanderForgeEngine6?.getStatus?.() || { state: 'starting', version: '6.0.0-alpha.3.1' };
  const engineStatus = v6.state === 'ready'
    ? `<div class="validation ok"><strong>Engine 6 ready.</strong> ${escapeHtml(v6.version || '6.0.0-alpha.3.1')} · Browser Web Worker${v6.lastSyncAt ? ' · table mirrored' : ''}${v6.lastCompile?.cards != null ? ` · ${v6.lastCompile.cards} card definitions analyzed${v6.lastCompile.unsupportedClauses ? ` · ${v6.lastCompile.unsupportedClauses} unsupported Oracle clause(s) kept for fallback` : ''}` : ''}</div>`
    : v6.state === 'error'
...
function startGame() {
  ...
  for (const playerId of ['p1', 'p2']) {
    ...
    if (playerId === bot.playerId && bot.enabled && next.players[playerId].name === 'Player 2') next.players[playerId].name = 'Autoplay Bot';
  ...
  ui.drawer = null;
  ui.mulliganBottomSelections = { p1: new Set(), p2: new Set() };
  toast(bot.enabled ? 'Game started. The Autoplay Bot will handle Player 2 after mulligans.' : 'Game started. Drag cards or tap one for its side menu.');
  if (bot.enabled) botSchedule(150);
}
