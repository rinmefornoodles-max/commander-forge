...
function canActivateManaChoice(card, choice, player = null) {
  ...
  const haste = (card.keywords || []).includes('Haste') || /\bhaste\b/i.test(card.oracleText || '');
...
function keywordSet(card) {
  const set = new Set((card?.keywords || []).map((keyword) => lower(keyword)));
...
function derivedCardState(card, battlefield = [], context = {}) {
  ...
  power += Number(counters['+1/+1'] || 0) - Number(counters['-1/-1'] || 0) + Number(card?.temporaryPowerBonus || 0);
  toughness += Number(counters['+1/+1'] || 0) - Number(counters['-1/-1'] || 0) + Number(card?.temporaryToughnessBonus || 0);
...
function landPlayLegality(state, playerId, card) {
  ...
  const allowance = 1 + extraLandAllowance(state, playerId);
...
function planSpellPayment(state, playerId, card, sourceZone = 'hand', options = {}) {
  ...
  for (let count = 0; count <= maxSpecial; count += 1) {
    ...
    const manaPlan = planManaPayment(player, adjustedCost, 0, { preserveColors, spellCard: card, sourceZone, excludeSourceIds: resources.map((item) => item.instanceId) });
...
function activatedAbilityActions(state, playerId, card) {
  const player = state.players[playerId];
  const rules = analyzeCardRules(card);
  const actions = [];
  rules.activatedAbilities.forEach((ability, abilityIndex) => {
    ...
    const paymentPlan = ability.manaCost
      ? planManaPayment(player, ability.manaCost, 0, { excludeSourceIds: ability.tap ? [card.instanceId] : [] })
...
function generateTacticalActions(state, playerId = state.activePlayerId, options = {}) {
  ...
  for (const card of player.zones.battlefield || []) actions.push(...activatedAbilityActions(state, playerId, card));
...
const { applySpellPayment, attackLegality, canPayMana, landEntryPlan, maximumHandSize, moveLegality, spellCastLegality, spendMana, stackDestination } = __modules["./rules.js"];
...
function moveCard(instanceId, targetPlayerId, targetZone, { force = false, libraryPosition = 'top', enterTapped = false } = {}) {
...
const MULTIPLAYER_APP_VERSION = '5.14.0';
const multiplayer = {
  mode: 'solo',
  role: null,
  localPlayerId: null,
  ...
  pendingJoinCode: '',
  lastError: '',
  publicReveals: { p1: { hand: {}, libraryTop: null }, p2: { hand: {}, libraryTop: null } },
};
...
function hiddenNetworkCard(playerId, zone, index) {
  return {
    ...
    power: '', toughness: '', keywords: [], colors: [], colorIdentity: [], legalities: {},
...
function emptyPublicRevealState() {
  return { p1: { hand: {}, libraryTop: null }, p2: { hand: {}, libraryTop: null } };
}
...
function ensureRevealBucket(playerId) {
  ...
  multiplayer.publicReveals[playerId] ||= { hand: {}, libraryTop: null };
  multiplayer.publicReveals[playerId].hand ||= {};
  return multiplayer.publicReveals[playerId];
}
...
function applyPublicReveal(reveal, { notify = true } = {}) {
  ...
  if (reveal.zone === 'hand') bucket.hand[reveal.card.instanceId || reveal.id] = deepClone(reveal);
  if (reveal.zone === 'library') bucket.libraryTop = deepClone(reveal);
  if (notify) {
...
function clearPublicReveal(revealId, playerId = null) {
  ...
  for (const id of playerId ? [playerId] : Object.keys(multiplayer.publicReveals)) {
    ...
    for (const [key, reveal] of Object.entries(bucket.hand || {})) {
      if (key === revealId || reveal?.id === revealId || reveal?.card?.instanceId === revealId) delete bucket.hand[key];
    }
    if (bucket.libraryTop && [bucket.libraryTop.id, bucket.libraryTop.card?.instanceId].includes(revealId)) bucket.libraryTop = null;
  }
...
function publishPublicReveal(card, playerId, zone) {
  if (!card || !playerId || !['hand', 'library'].includes(zone)) return;
  const reveal = {
    id: uid('network-reveal'),
    playerId,
    zone,
    position: zone === 'library' ? 'top' : null,
...
function revealRecordStillValid(reveal) {
  if (!reveal?.card?.instanceId) return false;
  const player = getState().players?.[reveal.playerId];
  if (!player) return false;
  if (reveal.zone === 'hand') return player.zones.hand.some((card) => card.instanceId === reveal.card.instanceId);
  if (reveal.zone === 'library') return player.zones.library[0]?.instanceId === reveal.card.instanceId;
  return false;
}

function reconcileLocalPublicReveals() {
  if (!isOnlineMultiplayer() || !multiplayer.localPlayerId) return;
  const bucket = ensureRevealBucket(multiplayer.localPlayerId);
  const records = [...Object.values(bucket.hand || {}), bucket.libraryTop].filter(Boolean);
  for (const reveal of records) {
    if (revealRecordStillValid(reveal)) continue;
    clearPublicReveal(reveal.id, multiplayer.localPlayerId);
    if (isMultiplayerConnected()) sendNetwork({ type: 'public-reveal-clear', revealId: reveal.id, playerId: multiplayer.localPlayerId });
  }
}
...
function knownLibraryTopReveal(state, playerId) {
  const direct = ensureRevealBucket(playerId).libraryTop?.card;
  if (direct) return direct;
  return state.knowledge?.players?.[playerId]?.knownLibraryTop?.[0]?.card || null;
}

function renderPublicRevealCard(card, label = 'Revealed') {
  return `<article class="game-card publicly-revealed-card" title="${escapeHtml(card.name)} · ${escapeHtml(label)}"><img src="${escapeHtml(publicRevealImage(card))}" alt="${escapeHtml(card.name)}" draggable="false" onerror="this.src='./card-back.svg'" /><span class="public-reveal-badge">${escapeHtml(label)}</span><div class="card-name-strip">${escapeHtml(card.name)}</div></article>`;
}

function stateForNetwork() {
  // Never transmit hidden hand or library identities. Only counts/placeholders and explicitly public knowledge leave this browser.
  const outgoing = deepClone(getState());
  outgoing.selected = null;
  for (const playerId of Object.keys(outgoing.players || {})) {
    outgoing.players[playerId].zones.hand = hiddenNetworkZone(playerId, 'hand', outgoing.players[playerId].zones.hand.length);
    outgoing.players[playerId].zones.library = hiddenNetworkZone(playerId, 'library', outgoing.players[playerId].zones.library.length);
  }
  return outgoing;
}
...
function mergeNetworkState(incoming) {
  ...
  if (localId && current.players?.[localId] && next.players?.[localId]) {
    next.players[localId].zones.hand = current.players[localId].zones.hand;
    next.players[localId].zones.library = current.players[localId].zones.library;
  }
...
function createOnlineInitialState(localPlayerId, localDraft, remotePublicDraft) {
  const next = createInitialState();
  next.settings.autoDraw = false;
  ...
  next.log.unshift({ id: uid('log'), time: new Date().toISOString(), text: 'Online game created. Each player drew their own opening hand.' });
  return next;
}
...
function startOnlineGame() {
  ...
  render();
  toast('Online game started. Each player controls their own side.');
}

function receiveOnlineStart(message) {
  multiplayer.publicReveals = emptyPublicRevealState();
  ui.publicRevealNotice = null;
  if (multiplayer.role !== 'guest' || !ui.drafts.p2.ready) return toast('Choose and validate Player 2’s deck before the host starts.', true);
  multiplayer.remoteDraft = message.hostDraft;
  multiplayer.suppressSync = true;
  multiplayer.applyingRemote = true;
  try {
    const next = createOnlineInitialState('p2', ui.drafts.p2, message.hostDraft);
    const publicState = deepClone(message.state);
    next.activePlayerId = publicState.activePlayerId;
    next.turnNumber = publicState.turnNumber;
    next.phaseIndex = publicState.phaseIndex;
    next.stack = publicState.stack;
    next.pendingTriggers = publicState.pendingTriggers;
    next.players.p1 = publicState.players.p1;
    next.players.p2.life = publicState.players.p2.life;
    next.players.p2.poison = publicState.players.p2.poison;
    next.players.p2.commanderDamage = publicState.players.p2.commanderDamage;
    setState(next);
...
function multiplayerActionAllowed(button, action) {
  ...
  const localId = multiplayer.localPlayerId;
  const safeActions = new Set(['coach', 'run-coach', 'close-inspector', 'open-log', 'close-log', 'open-settings', 'close-settings', 'open-setup', 'close-setup', 'open-zone', 'close-drawer', 'open-damage', 'close-damage', 'export-save', 'close-public-reveal']);
...
function renderOnlineSetup(state) {
  ...
  const footer = multiplayer.role === 'host'
    ...
    : `<div class="validation ${localReady ? 'ok' : ''}">${localReady ? 'Your deck is ready. Waiting for the host to start.' : 'Choose and validate your Player 2 deck.'}</div>`;
  return `${connectionPanel}<div class="setup-grid">${left}${right}</div><div class="setup-footer"><span class="small muted">Opponent hands and libraries stay hidden. Each player controls their own cards.</span>${footer}</div>`;
}
...
function isStackableBasicLand(card) {
...
function groupBattlefieldLands(lands) {
  const groups = [];
  const grouped = new Map();
  for (const card of lands || []) {
    if (!isStackableBasicLand(card)) {
      groups.push({ key: card.instanceId, cards: [card], stacked: false });
      continue;
    }
    const key = `${String(card.name || '').trim().toLocaleLowerCase()}|${card.tapped ? 'tapped' : 'untapped'}|${card.controller || card.owner || ''}`;
    let group = grouped.get(key);
    if (!group) {
      group = { key, cards: [], stacked: true };
...
function renderBattlefieldLandGroup(group, state) {
  ...
  return `<div class="basic-land-stack ${tapped ? 'tapped-stack' : 'untapped-stack'}" title="${escapeHtml(representative.name)} ×${count} · ${tapped ? 'tapped' : 'untapped'}">
    <span class="basic-land-stack-layer layer-one" aria-hidden="true"></span>
    <span class="basic-land-stack-layer layer-two" aria-hidden="true"></span>
    ${renderCard(representative, state)}
    <span class="basic-land-stack-count" aria-label="${count} copies">×${count}</span>
  </div>`;
}

function renderBattlefieldLandGroups(groups, state) {
  return (groups || []).map((group) => renderBattlefieldLandGroup(group, state)).join('');
}
...
function renderPlayerMat(state, playerId, opponent) {
  ...
  const otherPermanents = visibleBattlefield.filter((card) => !isLand(card));
  const lands = visibleBattlefield.filter((card) => isLand(card));
  const landGroups = groupBattlefieldLands(lands);
  const renderedLandGroups = renderBattlefieldLandGroups(landGroups, state);
  const tokenCount = battlefield.filter((card) => card.token && !card.attachedTo).length;
  const hiddenOtherTokenCount = hiddenTokens ? battlefield.filter((card) => card.token && !card.attachedTo && !isLand(card)).length : 0;
  const hiddenLandTokenCount = hiddenTokens ? battlefield.filter((card) => card.token && !card.attachedTo && isLand(card)).length : 0;
      ...
      <div class="board-main ${opponent ? 'opponent-board' : 'you-board'}">
        ...
        <div class="zone battlefield-zone" data-drop-zone="battlefield" data-player-id="${playerId}">
          ...
          <div class="battlefield-lanes ${opponent ? 'mirrored-battlefield-lanes' : ''}">
            ${opponent ? `
            <div class="battlefield-lane land-lane" data-drop-zone="battlefield" data-player-id="${playerId}">
              <span class="battlefield-lane-label">Lands</span>
              <div class="card-row ${landGroups.length < 4 ? 'centered' : ''}">${renderedLandGroups}${hiddenLandTokenCount ? `<span class="hidden-token-placeholder">${hiddenLandTokenCount} token${hiddenLandTokenCount === 1 ? '' : 's'} hidden</span>` : ''}</div>
            </div>
            <div class="battlefield-lane permanent-lane" data-drop-zone="battlefield" data-player-id="${playerId}">
              <span class="battlefield-lane-label">Other permanents</span>
              <div class="card-row ${otherPermanents.length < 4 ? 'centered' : ''}">${otherPermanents.map((card) => renderCard(card, state)).join('')}${hiddenOtherTokenCount ? `<span class="hidden-token-placeholder">${hiddenOtherTokenCount} token${hiddenOtherTokenCount === 1 ? '' : 's'} hidden</span>` : ''}</div>
            </div>` : `
            <div class="battlefield-lane permanent-lane" data-drop-zone="battlefield" data-player-id="${playerId}">
              <span class="battlefield-lane-label">Other permanents</span>
              <div class="card-row ${otherPermanents.length < 4 ? 'centered' : ''}">${otherPermanents.map((card) => renderCard(card, state)).join('')}${hiddenOtherTokenCount ? `<span class="hidden-token-placeholder">${hiddenOtherTokenCount} token${hiddenOtherTokenCount === 1 ? '' : 's'} hidden</span>` : ''}</div>
...
function renderHiddenHand(state, playerId, count) {
  ...
  const revealedHtml = revealed.map((card) => renderPublicRevealCard(card, 'Known in hand')).join('');
...
function renderCard(card, state, { compact = false } = {}) {
  ...
  const chargeCount = Number(card.counters?.charge || 0);
  const pageCount = Number(card.counters?.page || 0);
  if (chargeCount > 0) badges.push(`<span class="card-badge purple" title="${chargeCount} charge counter${chargeCount === 1 ? '' : 's'}">⚡${chargeCount}</span>`);
  if (pageCount > 0) badges.push(`<span class="card-badge purple" title="${pageCount} page counter${pageCount === 1 ? '' : 's'}">📖${pageCount}</span>`);
  const counterTotal = Object.entries(card.counters || {}).reduce((sum, [type, value]) => ['charge', 'page'].includes(String(type).toLowerCase()) ? sum : sum + Number(value || 0), 0);
...
function renderCoachInspector(state) {
  ...
  const auditHtml = audit ? `<details class="coach-audit"><summary>Information used by the coach</summary><div class="small"><div>✓ Your full hand and public zones</div><div>✓ Both visible battlefields, derived stats, keywords, attachments, and legal mana sources</div><div>✓ Public game memory and opponent hand size/behavior</div><div>✗ No opponent hidden card names or hidden library identities</div></div></details>` : '';
  return `<div class="inspector-section" style="display:flex;justify-content:space-between;align-items:center"><h3>Tactical information-set coach</h3><button class="icon-btn" data-action="close-inspector">×</button></div><div class="coach-panel"><p class="small muted">Searches legal plays and short sequences using exact tapped mana sources, derived power/toughness, keywords, attachments, combat restrictions, activated abilities, public memory, and sampled hidden interaction. It never reads the opponent's hidden hand or decklist.</p><button class="btn primary wide" data-action="run-coach">Analyze ${escapeHtml(active.name)}'s position</button><p class="small">${basicMoves.length} rules-checked tactical move and sequence candidates · ${state.settings.coachRollouts} samples per candidate.</p>${resultsHtml}${auditHtml}</div>${defenseHtml}`;
}
...
function renderRemoteLibraryBacks(state, playerId, count) {
  const revealedTop = knownLibraryTopReveal(state, playerId);
  const hiddenCount = Math.max(0, count - (revealedTop ? 1 : 0));
  const visible = Math.min(7, hiddenCount);
  return `${revealedTop ? renderPublicRevealCard(revealedTop, 'Revealed top') : ''}${Array.from({ length: visible }, () => `<div class="game-card card-back-stack"><img src="./card-back.svg" alt="Hidden library card" /></div>`).join('')}<div class="validation"><strong>${revealedTop ? 'Only the publicly revealed top card is visible.' : 'Library is hidden.'}</strong><br />Hidden card identities and library order never leave the owner’s browser.</div>`;
}
...
function renderModals(state) {
  return `${ui.setupOpen ? renderSetupModal(state) : ''}${ui.settingsOpen ? renderSettingsModal(state) : ''}${ui.tokenOpen && !ui.tokenPeek ? renderTokenModal(state) : ''}${ui.tokenOpen && ui.tokenPeek ? renderTokenPeekReturn() : ''}${state.openingHands?.active ? renderMulliganModal(state) : ''}${ui.damageOpen ? renderDamageModal(state) : ''}${ui.logOpen ? renderLogModal(state) : ''}${ui.publicRevealNotice ? renderPublicRevealModal(state) : ''}`;
}
...
function renderPublicRevealModal(state) {
  ...
  const locationLabel = reveal.zone === 'library' ? 'the top of their library' : 'their hand';
...
function renderSetupModal(state) {
  ...
  return `<div class="modal-backdrop"><section class="modal"><header class="modal-header"><div><h2>Set up the Commander table</h2><div class="small muted">Practice alone or connect directly to one other player.</div></div>${state.started ? '<button class="icon-btn" data-action="close-setup">×</button>' : ''}</header><div class="modal-body"><div class="setup-mode-switch"><button class="${solo ? 'active' : ''}" data-action="setup-mode" data-mode="solo">Solo practice</button><button class="${!solo ? 'active' : ''}" data-action="setup-mode" data-mode="online">Online multiplayer</button></div>${content}<input id="save-file-input" class="hidden" type="file" accept="application/json" /></div></section></div>`;
}
...
app.addEventListener('click', async (event) => {
  ...
  try {
    ...
    if (action === 'reveal-top') { const result = revealTopPublicly(button.dataset.playerId); handleResult(result); if (result?.ok) publishPublicReveal(result.card, button.dataset.playerId, 'library'); ui.libraryReveal = { playerId: button.dataset.playerId }; render(); }
    ...
    if (action === 'reveal-public') { const located = findCard(button.dataset.cardId, getState()); const result = revealCardPublicly(button.dataset.cardId); handleResult(result); if (result?.ok && located && ['hand', 'library'].includes(located.zone)) publishPublicReveal(located.card, located.card.owner || located.playerId, located.zone); }
