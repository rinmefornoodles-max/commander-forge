...
const MULTIPLAYER_APP_VERSION = '5.23.0';
const multiplayer = {
  mode: 'solo',
  role: null,
  localPlayerId: null,
...
function renderPlayerMat(state, playerId, opponent) {
  ...
  const resourceCorners = opponent
    ? `<div class="resource-corner land-corner"><span class="resource-corner-label">Lands</span><div class="card-row ${landGroups.length < 4 ? 'corner-aligned' : ''}">${renderedLandGroups}</div></div><div class="resource-corner utility-corner"><span class="resource-corner-label">Enchantments & noncreature tokens</span><div class="card-row ${utilityGroups.length < 4 ? 'corner-aligned-end' : ''}">${renderedUtilityGroups}${hiddenUtilityTokenCount ? `<span class="hidden-token-placeholder">${hiddenUtilityTokenCount} token${hiddenUtilityTokenCount === 1 ? '' : 's'} hidden</span>` : ''}</div></div>`
    : `<div class="resource-corner utility-corner"><span class="resource-corner-label">Enchantments & noncreature tokens</span><div class="card-row ${utilityGroups.length < 4 ? 'corner-aligned' : ''}">${renderedUtilityGroups}${hiddenUtilityTokenCount ? `<span class="hidden-token-placeholder">${hiddenUtilityTokenCount} token${hiddenUtilityTokenCount === 1 ? '' : 's'} hidden</span>` : ''}</div></div><div class="resource-corner land-corner"><span class="resource-corner-label">Lands</span><div class="card-row ${landGroups.length < 4 ? 'corner-aligned-end' : ''}">${renderedLandGroups}</div></div>`;
  return `
