...
function castForMutate(instanceId, targetId, position = 'top') {
  ...
  const playerId = source.card.controller || source.card.owner;
  const legality = spellCastLegality(current, playerId, source.card, 'hand', { alternativeManaCost: cost, useUntappedSources: current.settings.manaMode === 'auto' });
  ...
  updateState((draft) => {
    ...
    if (legality.payment?.ok) applySpellPayment(draft, playerId, legality.payment);
  ...
  }, { log: `${source.card.name} was cast for its mutate cost ${cost}, targeting ${target.card.name}.` });
...
function castAuraTargeting(instanceId, targetId) {
  const current = window.CommanderForge.getState();
  const found = findCard(instanceId, current);
  ...
  const playerId = found.card.controller || found.card.owner;
  return moveCard(instanceId, playerId, 'stack', { auraTargetId: targetId });
}
...
function moveCard(instanceId, targetPlayerId, targetZone, { force = false, libraryPosition = 'top', enterTapped = false, countsAsLandPlay = true, xValue = null, auraTargetId = null } = {}) {
  ...
  if (castAttempt && /\{X\}/i.test(source.card.manaCost || '') && xValue === null) {
...
app.addEventListener('click', async (event) => {
  ...
  try {
    ...
    if (action === 'cast-mutate') handleResult(castForMutate(button.dataset.cardId, button.dataset.targetId, button.dataset.position));
    ...
    if (action === 'cast-aura') handleResult(castAuraTargeting(button.dataset.cardId, button.dataset.targetId));
