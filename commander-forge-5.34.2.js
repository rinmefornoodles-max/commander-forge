...
function mutateCostForCard(card) { return keywordAbilityCost(card, 'mutate'); }
function isHumanCreature(card) { return /\bCreature\b/i.test(card?.typeLine || '') && /(?:—|-)\s*[^\n]*\bHuman\b/i.test(card?.typeLine || ''); }
...
function mutateStatus(instanceId) {
  const state = window.CommanderForge.getState();
  const found = findCard(instanceId, state);
  if (!found || found.zone !== 'hand' || !isCreature(found.card)) return { hasMutate: false, cost: '', targets: [], reason: '' };
  const cost = mutateCostForCard(found.card);
  if (!cost) return { hasMutate: false, cost: '', targets: [], reason: '' };
  ...
  const targets = ownedCreatures
    .filter((target) => !isHumanCreature(target))
    .map((target) => ({ targetId: target.instanceId, targetName: target.name, cost }));
  let reason = '';
  if (!targets.length) {
    reason = ownedCreatures.length
      ? 'You own creature(s) on the battlefield, but they are Human creatures. Mutate can only target a non-Human creature you own.'
      : 'You need a non-Human creature you own on the battlefield before you can cast this card for its mutate cost.';
  }
  return { hasMutate: true, cost, targets, reason };
}
...
function castForMutate(instanceId, targetId, position = 'top', { force = false } = {}) {
  ...
  if (!source || source.zone !== 'hand' || !isCreature(source.card)) return { ok: false, message: 'Mutate must be cast from a creature card in your hand.' };
  ...
  const playerId = source.card.controller || source.card.owner;
  const legality = force ? { legal: true, reasons: [], payment: null } : spellCastLegality(current, playerId, source.card, 'hand', { alternativeManaCost: cost, useUntappedSources: current.settings.manaMode === 'auto' });
  ...
  updateState((draft) => {
    ...
    spell.controller = playerId; spell.attacking = false; spell.castAsMutate = { targetId, position: position === 'under' ? 'under' : 'top', cost };
  ...
  }, { log: `${force ? '[TEST OVERRIDE] ' : ''}${source.card.name} was cast for its mutate cost ${cost}${force ? ' without paying' : ''}, targeting ${target.card.name}.` });
  return { ok: true, message: `${source.card.name} is on the stack as a mutating creature spell.` };
}
...
function moveCard(instanceId, targetPlayerId, targetZone, { force = false, libraryPosition = 'top', enterTapped = false, countsAsLandPlay = true, xValue = null, auraTargetId = null, skipPayment = false, testOverride = false } = {}) {
  ...
  updateState((draft) => {
    ...
    // A mutated permanent is one battlefield object but is made of multiple physical cards.
    // When it leaves the battlefield, every component changes zones separately.
    if (originalZone === 'battlefield' && targetZone !== 'battlefield' && (card.mutationPile || []).length > 1) {
      ...
      for (const part of parts) {
        ...
        } else owner.zones[targetZone].push(part);
        recordZoneTransition(draft, { card: part, actorId: targetPlayerId, subjectPlayerId: part.owner, fromZone: originalZone, toZone: targetZone, libraryPosition, castAttempt: false });
        enqueueBattlefieldLeaveEffects(draft, part, targetZone);
      }
      draft.selected = null;
      return;
    }
  ...
  if (card.castAsMutate) {
    updateState((draft) => {
      ...
      if (!target || target.zone !== 'battlefield' || target.card.owner !== spell.owner || !isCreature(target.card) || isHumanCreature(target.card)) { delete spell.castAsMutate; spell.summoningSick = true; spell.tapped = false; spell.enteredBattlefieldTurn = Number(draft.turnNumber || 1); draft.players[spell.controller].zones.battlefield.push(spell); enqueueBattlefieldEntryEffects(draft, spell, 'stack'); }
        ...
        for (const key of ['name','manaCost','manaValue','typeLine','power','toughness','colors','colorIdentity','image','imageSmall','scryfallId','oracleId','producedMana']) if (top[key] !== undefined) host[key] = deepClone(top[key]);
        host.keywords = [...new Set(components.flatMap((item) => item.keywords || []))]; host.oracleText = mergedMutationOracle(components); Object.assign(host, runtime); host.mutationPile = components; host.mutationTopIndex = topIndex; triggerMutationEffects(draft, host);
      }
...
return { attachmentTargetLegality, castAuraTargeting, testOverrideMove, changeControl, castPublicCardForFree, putPublicCardOntoBattlefield, playPublicLand, moveCard, removeToken, tapForMana, toggleTap, toggleAttack, addCounter, updateManualKeyword, createToken, copyAsToken, adjustPlayer, adjustCommanderDamage, adjustMana, clearMana, draw, mill, shuffleLibrary, nextPhase, setPhase, switchActivePlayer, resolveStackTop, counterStackTop, mulligan, keepOpeningHand, concede, queueManualEffect, activateBattlefieldAbility, setPendingEffectCondition, resolvePendingEffect, smartPendingEffectState, resolveSmartPendingEffect, resolveSmartPendingEffectNoResult, clearCombatMarkers, battlefieldActivatedAbilities, updateCardNote, flipCard, revealTop, revealTopPublicly, revealCardPublicly, resolvePrivateLibraryDecision, assignBlocker, attachCard, ninjutsuOptions, activateNinjutsu, mutateOptions, mutateStatus, castForMutate };
})();
...
const { addCounter, updateManualKeyword, assignBlocker, attachCard, attachmentTargetLegality, castAuraTargeting, testOverrideMove, changeControl, castPublicCardForFree, putPublicCardOntoBattlefield, playPublicLand, activateBattlefieldAbility, battlefieldActivatedAbilities, adjustCommanderDamage, adjustMana, adjustPlayer, clearMana, copyAsToken, counterStackTop, createToken, draw, flipCard, mill, moveCard, removeToken, mulligan, keepOpeningHand, concede, nextPhase, queueManualEffect, resolvePendingEffect, smartPendingEffectState, resolveSmartPendingEffect, resolveSmartPendingEffectNoResult, resolveStackTop, setPendingEffectCondition, clearCombatMarkers, revealCardPublicly, revealTopPublicly, resolvePrivateLibraryDecision, setPhase, shuffleLibrary, switchActivePlayer, toggleAttack, toggleTap, tapForMana, updateCardNote, ninjutsuOptions, activateNinjutsu, mutateOptions, mutateStatus, castForMutate } = __modules["./game.js"];
...
const MULTIPLAYER_APP_VERSION = '5.34.0';
const multiplayer = {
  mode: 'solo',
  role: null,
  localPlayerId: null,
...
function renderUndoApproval() {
  ...
  if (request.direction === 'incoming') {
    return `<div class="undo-approval-backdrop"><section class="undo-approval-card" role="dialog" aria-modal="true" aria-label="Undo request"><div class="undo-approval-icon">↶</div><h2>${escapeHtml(request.requesterName || 'Other player')} wants to undo the last action</h2><p>Allowing this restores their previous saved table state. Both players are notified before anything changes.</p><div class="undo-approval-actions"><button class="btn primary" data-action="approve-undo">Allow undo</button><button class="btn danger" data-action="deny-undo">Deny</button></div></section></div>`;
  }
  return `<div class="undo-request-toast"><strong>↶ Undo requested</strong><span>Waiting for the other player to approve…</span><button class="btn small-btn ghost" data-action="cancel-undo">Cancel</button></div>`;
}
...
function renderInspector(state) {
  ...
  const mutateInfo = selected.zone === 'hand' ? mutateStatus(card.instanceId) : { hasMutate: false, cost: '', targets: [], reason: '' };
  ...
  return `
    ...
    ${mutateInfo.hasMutate ? `<div class="inspector-section"><h3>Mutate ${escapeHtml(mutateInfo.cost)}</h3><p class="small muted">Cast this creature for its mutate cost onto a non-Human creature you own. Choose whether the mutating card goes on top of or underneath the target.</p>${mutateActions.length ? mutateActions.map((item) => `<div class="mana-choice-group"><div class="small muted wide">Target: ${escapeHtml(item.targetName)} · ${escapeHtml(item.cost)}</div><button class="btn primary" data-action="cast-mutate" data-card-id="${card.instanceId}" data-target-id="${item.targetId}" data-position="top">Mutate on top</button><button class="btn" data-action="cast-mutate" data-card-id="${card.instanceId}" data-target-id="${item.targetId}" data-position="under">Mutate underneath</button>${state.settings.testOverrides ? `<button class="btn ghost" data-action="cast-mutate" data-card-id="${card.instanceId}" data-target-id="${item.targetId}" data-position="top" data-test-override="true">Test mutate on top · no cost</button><button class="btn ghost" data-action="cast-mutate" data-card-id="${card.instanceId}" data-target-id="${item.targetId}" data-position="under" data-test-override="true">Test mutate underneath · no cost</button>` : ''}</div>`).join('') : `<div class="validation warn"><strong>Mutate is currently unavailable.</strong><br />${escapeHtml(mutateInfo.reason || 'No legal mutate target is available.')}</div>`}</div>` : ''}
...
function renderTestOverrideActions(card, selected, state) {
  ...
  return `<div class="inspector-section test-override-section"><h3>🧪 Test Override</h3><div class="validation warn"><strong>Sandbox controls</strong><br />These intentionally bypass mana, timing, land-play, and normal zone restrictions so you can build a board state or test a combo. They are not a claim that the move is legal in Magic.</div><div class="action-grid">${battlefieldButtons}<button class="btn ghost" data-action="test-force-move" data-card-id="${card.instanceId}" data-target-player-id="${ownerId}" data-zone="hand">Force owner hand</button><button class="btn ghost" data-action="test-force-move" data-card-id="${card.instanceId}" data-target-player-id="${ownerId}" data-zone="graveyard">Force owner graveyard</button><button class="btn ghost" data-action="test-force-move" data-card-id="${card.instanceId}" data-target-player-id="${ownerId}" data-zone="exile">Force owner exile</button><button class="btn ghost" data-action="test-force-move" data-card-id="${card.instanceId}" data-target-player-id="${controllerId}" data-zone="stack">Force stack / no cost</button><button class="btn ghost" data-action="test-force-move" data-card-id="${card.instanceId}" data-target-player-id="${ownerId}" data-zone="library" data-position="top">Force library top</button><button class="btn ghost" data-action="test-force-move" data-card-id="${card.instanceId}" data-target-player-id="${ownerId}" data-zone="library" data-position="bottom">Force library bottom</button></div><p class="small muted">Online play still limits these buttons to cards you are allowed to manipulate. Undo still requires the other player's approval.</p></div>`;
}
...
app.addEventListener('click', async (event) => {
  ...
  try {
    ...
    if (action === 'cast-mutate') handleResult(castForMutate(button.dataset.cardId, button.dataset.targetId, button.dataset.position, { force: button.dataset.testOverride === 'true' }));
    ...
    if (action === 'cast-aura') handleResult(castAuraTargeting(button.dataset.cardId, button.dataset.targetId, { force: button.dataset.testOverride === 'true' }));
    ...
    if (action === 'test-force-move') handleResult(testOverrideMove(button.dataset.cardId, button.dataset.targetPlayerId, button.dataset.zone, { libraryPosition: button.dataset.position || 'top' }));
