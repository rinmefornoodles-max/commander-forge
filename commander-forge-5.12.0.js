...
const MULTIPLAYER_APP_VERSION = '5.12.0';
const multiplayer = {
  mode: 'solo',
  role: null,
  localPlayerId: null,
...
function multiplayerActionAllowed(button, action) {
  ...
  const localId = multiplayer.localPlayerId;
  const safeActions = new Set(['coach', 'run-coach', 'close-inspector', 'open-log', 'close-log', 'open-settings', 'close-settings', 'open-setup', 'close-setup', 'open-zone', 'close-drawer', 'open-damage', 'close-damage', 'export-save']);
...
function renderModals(state) {
  return `${ui.setupOpen ? renderSetupModal(state) : ''}${ui.settingsOpen ? renderSettingsModal(state) : ''}${ui.tokenOpen && !ui.tokenPeek ? renderTokenModal(state) : ''}${ui.tokenOpen && ui.tokenPeek ? renderTokenPeekReturn() : ''}${state.openingHands?.active ? renderMulliganModal(state) : ''}${ui.damageOpen ? renderDamageModal(state) : ''}${ui.logOpen ? renderLogModal(state) : ''}`;
}
