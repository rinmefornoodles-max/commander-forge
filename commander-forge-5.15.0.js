...
const MULTIPLAYER_APP_VERSION = '5.15.0';
const multiplayer = {
  mode: 'solo',
  role: null,
  localPlayerId: null,
...
function stateForNetwork() {
  // Hidden zones are represented only by counts. Sending dozens of full card-back
  // placeholder objects can exceed browser data-channel message limits.
  ...
  for (const playerId of Object.keys(outgoing.players || {})) {
    outgoing.privateZoneCounts[playerId] = {
      hand: outgoing.players[playerId].zones.hand.length,
      library: outgoing.players[playerId].zones.library.length,
    };
    outgoing.players[playerId].zones.hand = [];
    outgoing.players[playerId].zones.library = [];
  }
  return outgoing;
}
