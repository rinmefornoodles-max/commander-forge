...
const { buildPlayerDeck, canUndo, createInitialState, createPlayer, drawCards, findCard, getState, importState, resetState, restore, setState, subscribe, undo, updateState } = __modules["./state.js"];
...
const MULTIPLAYER_APP_VERSION = '6.0.0-alpha.6.7-mp6';
...
function multiplayerStateChanged() {
  ...
  // Keep any old Solo save completely separate from the online lobby.
  // Gameplay sync starts only after an actual online game has been created.
...
function renderSettingsModal(state) {
  const v6 = window.CommanderForgeEngine6?.getStatus?.() || { state: 'starting', version: '6.0.0-alpha.6.7' };
  const engineStatus = v6.state === 'ready'
    ? `<div class="validation ok"><strong>Engine 6 ready.</strong> ${escapeHtml(v6.version || '6.0.0-alpha.6.7')} · Browser Web Worker${v6.lastSyncAt ? ' · table mirrored' : ''}${v6.lastCompile?.cards != null ? ` · ${v6.lastCompile.cards} card definitions analyzed${v6.lastCompile.unsupportedClauses ? ` · ${v6.lastCompile.unsupportedClauses} unsupported Oracle clause(s) kept for fallback` : ''}` : ''}</div>`
    : v6.state === 'error'
