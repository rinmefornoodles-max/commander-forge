...
function ensureStateShape(next) {
  ...
  next.settings = { ...DEFAULT_SETTINGS, ...previousSettings, manaAutomationV3: true, coachInformationSetV4: true, coachTacticalV5: true, phaseSafetyV6: true };
...
const { applySpellPayment, attackLegality, landEntryPlan, moveLegality, spellCastLegality, stackDestination } = __modules["./rules.js"];
    ...
    <section class="player-mat ${opponent ? 'opponent' : 'you'}" data-player-mat="${playerId}">
      ...
      <div class="board-main ${opponent ? 'opponent-board' : 'you-board'}">
        ...
        <div class="zone hand-zone" data-drop-zone="hand" data-player-id="${playerId}">
          <span class="zone-label">${escapeHtml(player.name)}'s hand · ${player.zones.hand.length}</span>
        ...
        ${!opponent ? `
        <div class="zone hand-zone" data-drop-zone="hand" data-player-id="${playerId}">
          <span class="zone-label">Your hand · ${player.zones.hand.length}</span>
