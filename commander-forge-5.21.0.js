...
const MULTIPLAYER_APP_VERSION = '5.21.0';
const multiplayer = {
  mode: 'solo',
  role: null,
  localPlayerId: null,
...
function multiplayerActionAllowed(button, action) {
  ...
  const localId = multiplayer.localPlayerId;
  const safeActions = new Set(['coach', 'run-coach', 'cancel-coach', 'close-inspector', 'open-log', 'close-log', 'open-settings', 'close-settings', 'open-setup', 'close-setup', 'open-zone', 'close-drawer', 'open-damage', 'close-damage', 'export-save', 'close-public-reveal', 'close-scry', 'inspect-revealed-card']);
...
function openPrivateLibraryTool(playerId, config = {}) {
  ...
  ui.scry = {
    ...
    items: library.slice(0, amount).map((card) => ({ cardId: card.instanceId, destination: defaultDestination, reveal: false })),
    allowedDestinations,
    allowReveal: config.allowReveal === true || (config.allowReveal !== false && mode === 'look'),
  ...
  if (result?.ok) {
    ...
    for (const item of tool.items.filter((entry) => entry.reveal)) {
      ...
      if (entry.destination === 'hand' || entry.destination === 'top' || entry.destination === 'bottom') {
        ...
        publishPublicReveal(entry.card, tool.playerId, entry.destination === 'hand' ? 'hand' : 'library', { position: entry.destination === 'hand' ? null : 'revealed' });
      }
    }
    ui.scry = null;
  }
...
function renderKnownRevealInspector(reveal) {
  ...
  const zoneLabel = reveal.zone === 'hand' ? 'Known in opponent hand' : 'Revealed from opponent library';
  return `
...
app.addEventListener('click', async (event) => {
  ...
  try {
    ...
    if (action === 'create-treasure') createTreasureToken();
    ...
    if (action === 'reveal-top') { const result = revealTopPublicly(button.dataset.playerId); handleResult(result); if (result?.ok) publishPublicReveal(result.card, button.dataset.playerId, 'library', { position: 'top' }); ui.libraryReveal = { playerId: button.dataset.playerId }; render(); }
...
function createTreasureToken() {
  const state = getState();
  const playerId = isOnlineMultiplayer()
    ? multiplayer.localPlayerId
    : state.activePlayerId;
  if (!playerId || !state.players[playerId]) {
    toast('Choose a player before creating a Treasure.', true);
    return;
  }
  ...
  toast(`${state.players[playerId].name} created a Treasure token.`);
}
