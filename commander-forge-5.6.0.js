...
function ensureStateShape(next) {
  ...
  next.settings = { ...DEFAULT_SETTINGS, ...previousSettings, hideOpponentHand: false, manaAutomationV3: true, coachInformationSetV4: true, coachTacticalV5: true, phaseSafetyV6: true, tabletopUXV7: true, tokenPeekV8: true, commanderZoneUXV9: true };
...
function restore() {
  try {
    ...
    if (![2, 3, 4, 5, 6, 7, 8].includes(parsed.version)) return false;
...
function importState(imported) {
  if (!imported || ![2, 3, 4, 5, 6, 7, 8].includes(imported.version) || !imported.players) throw new Error('This is not a compatible Commander Forge save file.');
  history = [];
    ...
    <section class="player-mat ${opponent ? 'opponent' : 'you'}" data-player-mat="${playerId}">
      <aside class="player-sidebar">
        ...
        <div class="command-slot zone ${player.zones.command.length ? '' : 'empty'}" data-drop-zone="command" data-player-id="${playerId}">
          ...
          <div class="commander-zone-list">${player.zones.command.map((card) => renderCommanderZoneCard(card, state, playerId)).join('') || `<span class="muted small">Drop commander here</span>`}</div>
        </div>
...
function renderCommanderZoneCard(card, state, playerId) {
  const player = state.players[playerId];
  const casts = Number(player.commanderCastCount?.[card.instanceId] || 0);
  const tax = casts * 2;
  return `<div class="commander-zone-card compact">
    ${renderCard(card, state, { compact: true })}
    <div class="commander-tax-stepper" aria-label="Commander tax +${tax}">
      <button type="button" data-action="adjust-commander-tax" data-card-id="${card.instanceId}" data-player-id="${playerId}" data-delta="-1" aria-label="Reduce commander tax" ${casts <= 0 ? 'disabled' : ''}>−</button>
      <span>Tax +${tax}</span>
      <button type="button" data-action="adjust-commander-tax" data-card-id="${card.instanceId}" data-player-id="${playerId}" data-delta="1" aria-label="Increase commander tax">+</button>
    </div>
  </div>`;
}

function adjustCommanderTax(playerId, cardId, delta) {
  const player = getState().players[playerId];
  if (!player || !player.zones.command.some((card) => card.instanceId === cardId)) return;
  const current = Number(player.commanderCastCount?.[cardId] || 0);
  const next = Math.max(0, current + Number(delta || 0));
  if (next === current) return;
  updateState((state) => {
    state.players[playerId].commanderCastCount[cardId] = next;
  }, { log: `${player.name}'s commander tax was adjusted to +${next * 2}.` });
}
...
app.addEventListener('click', async (event) => {
  ...
  try {
    ...
    if (action === 'adjust-commander-tax') adjustCommanderTax(button.dataset.playerId, button.dataset.cardId, Number(button.dataset.delta));
