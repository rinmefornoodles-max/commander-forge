...
function activateNinjutsu(instanceId, attackerId) {
  ...
  if (pendingNinjutsuAbility(current, instanceId)) return { ok: false, message: `${source.card.name} already has a ninjutsu ability waiting on the stack.` };
  ...
  const abilityId = uid('ninjutsu');
  updateState((draft) => {
    if (pendingNinjutsuAbility(draft, instanceId)) return;
    ...
    // Repair any stale duplicate of the Ninja itself while leaving the actual
    // card in hand until the ability resolves.
    const liveSource = extractUniqueTopLevelCard(draft, instanceId, 'hand');
    if (!liveSource || liveSource.fromZone !== 'hand') return;
    liveSource.card.controller = playerId;
    liveSource.card.ninjutsuPendingId = abilityId;
    draft.players[liveSource.card.owner].zones.hand.push(liveSource.card);

    draft.stack.push({ instanceId: abilityId, owner: source.card.owner, controller: playerId, name: `Ninjutsu — ${source.card.name}`, typeLine: 'Activated Ability', oracleText: `Ninjutsu ${option.cost}`, image: source.card.image, imageSmall: source.card.imageSmall, token: false, commander: false, _specialAbility: 'ninjutsu', sourceCardId: instanceId, sourceCardName: source.card.name, returnedAttackerId: attackerId });
    ...
    for (const permanent of draft.players[playerId].zones.battlefield) for (const line of oracleAbilityLines(permanent)) {
      if (/whenever you activate a ninjutsu ability/i.test(triggerConditionClause(line) || line)) queuePendingEffect(draft, { sourceCard: permanent, controllerId: playerId, kind: 'ninjutsu-trigger', text: line });
    }
...
function smartPendingEffectState(effect, state) {
  ...
  if (sacrificeImperative || sacrificeSubject) {
    ...
    if (subject !== 'that player') {
      const victimId = subject.includes('opponent') ? otherPlayerId(state, effect.controllerId) : effect.controllerId;
...
function resolveSmartPendingEffectNoResult(effectId) {
  ...
  if (smart.kind === 'search-library' && !smart.zeroAllowed) return { ok: false, message: 'The search has no matching card. If the library really has no legal match, use the manual resolve button.' };
...
function queuePendingEffect(draft, effectData) {
  ...
  const noChoiceKinds = new Set(['return-own-graveyard-creature', 'sacrifice-permanent', 'discard-card', 'hand-to-graveyard']);
...
function renderLibraryEffectPicker(state) {
  ...
  if (smart.kind !== 'search-library') return '';
...
function renderSettingsModal(state) {
  const v6 = window.CommanderForgeEngine6?.getStatus?.() || { state: 'starting', version: '6.0.0-alpha.4' };
  const engineStatus = v6.state === 'ready'
    ? `<div class="validation ok"><strong>Engine 6 ready.</strong> ${escapeHtml(v6.version || '6.0.0-alpha.4')} · Browser Web Worker${v6.lastSyncAt ? ' · table mirrored' : ''}${v6.lastCompile?.cards != null ? ` · ${v6.lastCompile.cards} card definitions analyzed${v6.lastCompile.unsupportedClauses ? ` · ${v6.lastCompile.unsupportedClauses} unsupported Oracle clause(s) kept for fallback` : ''}` : ''}</div>`
    : v6.state === 'error'
...
function renderMulliganModal(state) {
  ...
  if (onlineLocalId) {
    const remoteId = multiplayer.remotePlayerId;
    opponentStatus = `<div class="mulligan-private-status ${state.openingHands.kept[remoteId] ? 'ready' : ''}"><span class="mulligan-private-icon">🔒</span><div><strong>Opponent's opening hand stays private</strong><span>${state.openingHands.kept[remoteId] ? 'Opponent kept their hand.' : 'Waiting for opponent to keep or mulligan…'}</span></div></div>`;
  } else if (hideBotHand) {
...
app.addEventListener('click', async (event) => {
  ...
  try {
    ...
    if (action === 'resolve-library-effect') {
      const picker = ui.libraryEffectPicker;
      if (!picker) return;
      const result = resolveSmartLibraryEffect(picker.effectId, picker.selected || []);
      if (result?.ok) ui.libraryEffectPicker = null;
      handleResult(result);
    }
