...
function analyzeCardRules(card) {
  ...
  const signature = `${card.oracleText || card.oracle_text || ''}${card.typeLine || card.type_line || ''}${(card.keywords || []).join('|')}${(card.colors || []).join('|')}`;
...
function resolvePrivateLibraryDecision(playerId, cardIds = [], items = [], options = {}) {
  ...
  updateState((draft) => {
    ...
    for (const item of items) groups[item.destination].push(byId.get(item.cardId));
...
const MULTIPLAYER_APP_VERSION = '5.18.0';
const multiplayer = {
  mode: 'solo',
  role: null,
  localPlayerId: null,
...
function multiplayerActionAllowed(button, action) {
  ...
  const localId = multiplayer.localPlayerId;
  const safeActions = new Set(['coach', 'run-coach', 'cancel-coach', 'close-inspector', 'open-log', 'close-log', 'open-settings', 'close-settings', 'open-setup', 'close-setup', 'open-zone', 'close-drawer', 'open-damage', 'close-damage', 'export-save', 'close-public-reveal', 'close-scry']);
...
function openPrivateLibraryTool(playerId, config = {}) {
  ...
  ui.scry = {
    ...
    mode: config.mode || 'scry',
    label: config.label || `Scry ${amount}`,
    amount,
    cardIds: library.slice(0, amount).map((card) => card.instanceId),
    items: library.slice(0, amount).map((card) => ({ cardId: card.instanceId, destination: defaultDestination })),
    allowedDestinations,
...
function render(options = {}) {
  ...
  const setupBody = ui.setupOpen ? app.querySelector('.modal .modal-body') : null;
  if (setupBody) ui.setupScrollTop = setupBody.scrollTop;
  ...
  requestAnimationFrame(() => {
    ...
    const replacementSetupBody = ui.setupOpen ? app.querySelector('.modal .modal-body') : null;
    if (replacementSetupBody) replacementSetupBody.scrollTop = ui.setupScrollTop || 0;
...
function renderInspector(state) {
  ...
  return `
    ...
    <div class="inspector-section"><h3>Quick actions</h3><div class="action-grid">
      ...
      ${libraryActions.map((effect, index) => `<button class="btn" data-action="card-library-effect" data-card-id="${card.instanceId}" data-effect-index="${index}">👁 ${escapeHtml(effect.label)} privately</button>`).join('')}
...
function renderCoachInspector(state) {
  ...
  const analysisMeta = ui.coach
    ? `${ui.coach.results.length} of ${ui.coach.moves.length} rules-checked candidates analyzed · up to ${ui.coach.rollouts} samples each · ${(Number(ui.coach.elapsedMs || 0) / 1000).toFixed(1)}s${ui.coach.truncated ? ' · stopped at the responsiveness limit' : ''}.`
    : `Checks immediate plays and short sequences after you press Analyze · up to ${state.settings.coachRollouts} samples per candidate.`;
  return `<div class="inspector-section" style="display:flex;justify-content:space-between;align-items:center"><h3>Tactical information-set coach</h3><button class="icon-btn" data-action="close-inspector">×</button></div><div class="coach-panel"><p class="small muted">Searches legal plays and short sequences using exact tapped mana sources, derived power/toughness, keywords, attachments, combat restrictions, activated abilities, public memory, and sampled hidden interaction. It never reads the opponent's hidden hand or decklist.</p><button class="btn primary wide" data-action="run-coach" ${ui.coachRunning ? 'disabled' : ''}>${ui.coachRunning ? 'Analyzing…' : `Analyze ${escapeHtml(active.name)}'s position`}</button><p class="small">${analysisMeta}</p>${resultsHtml}${auditHtml}</div>${defenseHtml}`;
}
...
function renderPrivateLibraryModal(state) {
  ...
  const cards = tool.items.map((item) => {
    ...
    return `<article class="scry-card"><img src="${escapeHtml(cardImage(card))}" alt="${escapeHtml(card.name)}" onerror="this.src='./card-back.svg'" /><h3>${escapeHtml(card.name)}</h3><div class="scry-destinations">${tool.allowedDestinations.map((destination) => `<button class="${item.destination === destination ? 'active' : ''}" data-action="scry-destination" data-card-id="${card.instanceId}" data-destination="${destination}">${destinationLabels[destination]}</button>`).join('')}</div><div class="scry-order"><button class="btn small-btn" data-action="scry-order" data-card-id="${card.instanceId}" data-direction="up" ${place <= 0 ? 'disabled' : ''}>← Earlier</button><span>${escapeHtml(destinationLabels[item.destination])} #${place + 1}</span><button class="btn small-btn" data-action="scry-order" data-card-id="${card.instanceId}" data-direction="down" ${place >= sameDestination.length - 1 ? 'disabled' : ''}>Later →</button></div></article>`;
  }).join('');
