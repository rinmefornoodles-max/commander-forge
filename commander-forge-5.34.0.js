...
function mutateOptions(instanceId) {
  ...
  if (!cost) return [];
  return Object.values(state.players).flatMap((player) => player.zones.battlefield || [])
    .filter((target) => target.owner === found.card.owner && isCreature(target) && !isHumanCreature(target))
...
function renderInspector(state) {
  ...
  const mutateActions = selected.zone === 'hand' ? mutateOptions(card.instanceId) : [];
  ...
  return `
    ...
    ${mutateActions.length ? `<div class="inspector-section"><h3>Mutate</h3><p class="small muted">Cast for the mutate cost and choose whether this card goes on top of or underneath the target creature.</p>${mutateActions.map((item) => `<div class="mana-choice-group"><div class="small muted wide">Target: ${escapeHtml(item.targetName)} · ${escapeHtml(item.cost)}</div><button class="btn primary" data-action="cast-mutate" data-card-id="${card.instanceId}" data-target-id="${item.targetId}" data-position="top">Mutate on top</button><button class="btn" data-action="cast-mutate" data-card-id="${card.instanceId}" data-target-id="${item.targetId}" data-position="under">Mutate underneath</button></div>`).join('')}</div>` : ''}
