...
const { applySpellPayment, attackGroupLegality, attackLegality, attackTaxPlan, landEntryPlan, landPlayLegality, manaDevelopmentSnapshot, planManaPayment, spellCastLegality } = __modules["./rules.js"];
...
const { applySpellPayment, attackLegality, attackTaxPlan, canPayMana, landEntryPlan, landPlayLegality, maximumHandSize, moveLegality, planManaPayment, spellCastLegality, spendMana, stackDestination } = __modules["./rules.js"];
...
// ---- main.js ----
globalThis.CommanderForgeBuildVersion = '6.11.2';
(() => {
...
const MULTIPLAYER_APP_VERSION = '6.11.2-mp6';
...
function renderOracleCompilerDiagnostic(card) {
  ...
  if ((!program?.steps?.length) && !attackCosts.length) return '';
  ...
  return `<div class="compiler-status ${(program?.authoritative || attackCosts.length) ? 'ready' : 'partial'}">
...
function renderSettingsModal(state) {
  const v6 = window.CommanderForgeEngine6?.getStatus?.() || { state: 'starting', version: '6.11.2' };
  const engineStatus = v6.state === 'ready'
    ? `<div class="validation ok"><strong>Engine 6 ready.</strong> ${escapeHtml(v6.version || '6.11.2')} · Browser Web Worker${v6.lastSyncAt ? ' · table mirrored' : ''}${v6.lastCompile?.cards != null ? ` · ${v6.lastCompile.cards} card definitions analyzed${v6.lastCompile.unsupportedClauses ? ` · ${v6.lastCompile.unsupportedClauses} unsupported Oracle clause(s) kept for fallback` : ''}` : ''}</div>`
    : v6.state === 'error'
