...
const MULTIPLAYER_APP_VERSION = '6.0.0-alpha.6.9.1-mp6';
...
function renderSettingsModal(state) {
  const v6 = window.CommanderForgeEngine6?.getStatus?.() || { state: 'starting', version: '6.0.0-alpha.6.9.1' };
  const engineStatus = v6.state === 'ready'
    ? `<div class="validation ok"><strong>Engine 6 ready.</strong> ${escapeHtml(v6.version || '6.0.0-alpha.6.9.1')} · Browser Web Worker${v6.lastSyncAt ? ' · table mirrored' : ''}${v6.lastCompile?.cards != null ? ` · ${v6.lastCompile.cards} card definitions analyzed${v6.lastCompile.unsupportedClauses ? ` · ${v6.lastCompile.unsupportedClauses} unsupported Oracle clause(s) kept for fallback` : ''}` : ''}</div>`
    : v6.state === 'error'
