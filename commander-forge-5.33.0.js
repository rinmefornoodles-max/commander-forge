...
function castForMutate(instanceId, targetId, position = 'top') {
  ...
  updateState((draft) => {
    ...
    draft.stack.push(spell); draft.players[playerId].spellsCastThisTurn = Number(draft.players[playerId].spellsCastThisTurn || 0) + 1; draft.priorityPlayerId = playerId; draft.consecutivePasses = 0;
...
function resolveSmartPendingEffect(effectId, cardId) {
  const current = window.CommanderForge.getState(); const effect = current.pendingTriggers.find((item) => item.id === effectId);
  if (!effect) return { ok: false, message: 'Pending effect not found.' }; const text = String(effect.text || '');
  if (/return target creature card from your graveyard to the battlefield/i.test(text)) {
    const target = findCard(cardId, current);
    if (!target || target.zone !== 'graveyard' || target.playerId !== effect.controllerId || !isCreature(target.card)) return { ok: false, message: 'Choose a creature card from your graveyard.' };
    updateState((draft) => { const located = findCard(cardId, draft); const card = located.container.splice(located.index, 1)[0]; card.controller = effect.controllerId; card.tapped = false; card.summoningSick = true; card.enteredBattlefieldTurn = Number(draft.turnNumber || 1); draft.players[effect.controllerId].zones.battlefield.push(card); enqueueBattlefieldEntryEffects(draft, card, 'graveyard'); draft.pendingTriggers = draft.pendingTriggers.filter((item) => item.id !== effectId); }, { log: `${effect.sourceName}: returned ${target.card.name} from the graveyard to the battlefield.` });
    return { ok: true };
  }
  if (/\b(?:that player|target opponent|each opponent) sacrifices? a creature\b/i.test(text)) {
    const target = findCard(cardId, current); const victimId = current.activePlayerId;
    if (!target || target.zone !== 'battlefield' || target.card.controller !== victimId || !isCreature(target.card)) return { ok: false, message: 'Choose a creature controlled by the affected player.' };
    updateState((draft) => { const located = findCard(cardId, draft); const card = located.container.splice(located.index, 1)[0]; clearCardRelations(draft, card); draft.players[card.owner].zones.graveyard.push(card); enqueueBattlefieldLeaveEffects(draft, card, 'graveyard'); draft.pendingTriggers = draft.pendingTriggers.filter((item) => item.id !== effectId); }, { log: `${effect.sourceName}: ${target.card.name} was sacrificed.` });
    return { ok: true };
  }
...
function moveCard(instanceId, targetPlayerId, targetZone, { force = false, libraryPosition = 'top', enterTapped = false, countsAsLandPlay = true, xValue = null } = {}) {
...
function resolveStackTop() {
  ...
  updateState((draft) => { const resolved = draft.stack.pop(); if (destination === 'battlefield') { resolved.summoningSick = isCreature(resolved); draft.players[resolved.controller].zones.battlefield.push(resolved); enqueueBattlefieldEntryEffects(draft, resolved, 'stack'); } else draft.players[resolved.owner].zones.graveyard.push(resolved); draft.priorityPlayerId = draft.activePlayerId; draft.consecutivePasses = 0; recordPublicEvent(draft, { type: 'resolved', actorId: resolved.controller, subjectPlayerId: resolved.owner, card: resolved, fromZone: 'stack', toZone: destination, meaningful: true }); }, { log: `${card.name} resolved to ${ZONE_LABELS[destination]}.` });
  return { ok: true };
}
...
const MULTIPLAYER_APP_VERSION = '5.31.0';
const multiplayer = {
  mode: 'solo',
  role: null,
  localPlayerId: null,
...
function renderSmartEffectActions(effect, state) {
  ...
  if (/return target creature card from your graveyard to the battlefield/i.test(text)) {
    ...
    if (!cards.length) return '<span class="small muted">No creature cards are currently in your graveyard.</span>';
    return `<div class="mana-choice-group"><div class="small muted wide">Choose the creature to return</div>${cards.slice(0, 20).map((card) => `<button class="btn small-btn primary" data-action="smart-effect-card" data-effect-id="${effect.id}" data-card-id="${card.instanceId}">Return ${escapeHtml(card.name)}</button>`).join('')}</div>`;
  }
  if (/\b(?:that player|target opponent|each opponent) sacrifices? a creature\b/i.test(text)) {
    const victim = state.players[state.activePlayerId]; const cards = (victim?.zones?.battlefield || []).filter(isCreature);
    if (!cards.length) return `<span class="small muted">${escapeHtml(victim?.name || 'Affected player')} controls no creatures to sacrifice.</span>`;
    return `<div class="mana-choice-group"><div class="small muted wide">${escapeHtml(victim.name)} chooses a creature to sacrifice</div>${cards.slice(0, 20).map((card) => `<button class="btn small-btn danger" data-action="smart-effect-card" data-effect-id="${effect.id}" data-card-id="${card.instanceId}">Sacrifice ${escapeHtml(card.name)}</button>`).join('')}</div>`;
  }
  return '';
}
