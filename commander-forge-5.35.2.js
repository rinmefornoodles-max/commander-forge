...
const DEFAULT_SETTINGS = {
  ...
  tokenPeekV8: true,
  testOverrides: true,
  externalRulesEngineMode: 'local',
  externalRulesEngineEndpoint: '',
};
...
function triggerMutationEffects(draft, host) {
  ...
  for (const source of components) for (const line of oracleAbilityLines(source)) {
    if (/whenever this creature mutates/i.test(line)) queuePendingEffect(draft, { sourceCard: host, controllerId: host.controller, kind: 'mutate-trigger', text: line });
  }
}

function activateNinjutsu(instanceId, attackerId) {
  const current = window.CommanderForge.getState();
  const source = findCard(instanceId, current); const attacker = findCard(attackerId, current);
  ...
  if (!option || !attacker || attacker.card.controller !== playerId) return { ok: false, message: 'Choose one of your unblocked attacking creatures after blockers are declared.' };
  ...
  updateState((draft) => {
    ...
    const returned = findCard(attackerId, draft); if (!returned) return;
    const returnedCard = returned.container.splice(returned.index, 1)[0]; clearCardRelations(draft, returnedCard);
    returnedCard.attacking = false; returnedCard.tapped = false; returnedCard.blocking = null; returnedCard.blockedBy = [];
    draft.players[playerId].zones.hand.push(returnedCard);
    draft.stack.push({ instanceId: uid('ninjutsu'), owner: source.card.owner, controller: playerId, name: `Ninjutsu — ${source.card.name}`, typeLine: 'Activated Ability', oracleText: `Ninjutsu ${option.cost}`, image: source.card.image, imageSmall: source.card.imageSmall, token: false, commander: false, _specialAbility: 'ninjutsu', sourceCardId: instanceId, sourceCardName: source.card.name });
    draft.priorityPlayerId = playerId; draft.consecutivePasses = 0;
    for (const permanent of draft.players[playerId].zones.battlefield) for (const line of oracleAbilityLines(permanent)) {
      if (/whenever you activate a ninjutsu ability/i.test(line)) queuePendingEffect(draft, { sourceCard: permanent, controllerId: playerId, kind: 'ninjutsu-trigger', text: line });
    }
  }, { log: `${source.card.name}: activated ninjutsu ${option.cost}, returning ${attacker.card.name} to hand as the activation cost.` });
  return { ok: true, message: `${source.card.name}'s ninjutsu ability is on the stack.` };
}

function castForMutate(instanceId, targetId, position = 'top', { force = false } = {}) {
  const current = window.CommanderForge.getState(); const source = findCard(instanceId, current); const target = findCard(targetId, current);
  ...
  const cost = mutateCostForCard(source.card); if (!cost) return { ok: false, message: 'No mutate cost was found on that card.' };
  ...
  updateState((draft) => {
    const located = findCard(instanceId, draft); if (!located) return; const spell = located.container.splice(located.index, 1)[0];
    ...
    spell.controller = playerId; spell.attacking = false; spell.castAsMutate = { targetId, position: position === 'under' ? 'under' : 'top', cost, sourceZone, commanderTax };
    draft.stack.push(spell); draft.players[playerId].spellsCastThisTurn = Number(draft.players[playerId].spellsCastThisTurn || 0) + 1; draft.priorityPlayerId = playerId; draft.consecutivePasses = 0; emitGameEvent(draft, { type: 'cast', actorId: playerId, card: spell });
  }, { log: `${force ? '[TEST OVERRIDE] ' : ''}${source.card.name} was cast for its mutate cost ${cost}${commanderTax ? ` plus {${commanderTax}} commander tax` : ''}${force ? ' without paying mana' : ''}, targeting ${target.card.name}.` });
  return { ok: true, message: `${source.card.name} is on the stack as a mutating creature spell${commanderTax ? ` (mutate cost plus {${commanderTax}} commander tax)` : ''}.` };
}
...
function smartPendingEffectState(effect, state) {
  ...
  if (/\b(?:that player|target opponent|each opponent) sacrifices? a creature\b/i.test(text)) {
    const victimId = /\b(?:each opponent|target opponent)\b/i.test(text)
      ? otherPlayerId(state, effect.controllerId)
      : state.activePlayerId;
    const cards = (state.players?.[victimId]?.zones?.battlefield || []).filter(isCreature);
    return { kind: 'sacrifice-creature', cards, victimId, noResultReason: 'no-creature-to-sacrifice' };
  }
...
function resolveSmartPendingEffectNoResult(effectId) {
  ...
  updateState((draft) => {
    ...
    log: targetless
      ? `${effect.sourceName}: the triggered ability had no legal target and was removed without effect.`
      : `${effect.sourceName}: the effect resolved, but there was no creature to sacrifice.`,
  });
  return { ok: true, message: targetless ? 'No legal target; trigger removed.' : 'Resolved with no creature to sacrifice.' };
}
...
function resolveSmartPendingEffectMulti(effectId, cardIds = [], powerOverrides = {}) {
  ...
  if (!effect) return { ok: false, message: 'Pending effect not found.' };
  const smart = smartPendingEffectState(effect, current);
  if (smart.kind !== 'return-own-graveyard-creatures-total-power') return { ok: false, message: 'This effect is not a multi-card graveyard return.' };
  ...
  updateState((draft) => {
    ...
    for (const card of returned) enqueueBattlefieldEntryEffects(draft, card, 'graveyard');
    draft.pendingTriggers = draft.pendingTriggers.filter((item) => item.id !== effectId);
...
function resolveSmartPendingEffect(effectId, cardId) {
  ...
  if (smart.kind === 'sacrifice-creature') {
    if (!smart.cards.length) return resolveSmartPendingEffectNoResult(effectId);
    const target = findCard(cardId, current);
    if (!target || !smart.cards.some((card) => card.instanceId === cardId)) return { ok: false, message: 'Choose a creature controlled by the affected player.' };
    updateState((draft) => {
      const located = findCard(cardId, draft); const card = located.container.splice(located.index, 1)[0];
      clearCardRelations(draft, card);
      draft.players[card.owner].zones.graveyard.push(card);
      enqueueBattlefieldLeaveEffects(draft, card, 'graveyard');
      draft.pendingTriggers = draft.pendingTriggers.filter((item) => item.id !== effectId);
    }, { log: `${effect.sourceName}: ${target.card.name} was sacrificed.` });
    return { ok: true };
  }
...
function queuePendingEffect(draft, effectData) {
  ...
  if (['return-own-graveyard-creature', 'sacrifice-creature'].includes(smart.kind) && !smart.cards.length) {
    const targetless = smart.noResultReason === 'no-legal-target';
    draft.log.unshift({
      id: uid('log'),
      ...
      text: targetless
        ...
        : `${effect.sourceName}: effect had no creature to sacrifice and resolved with no effect.`,
    });
...
function triggerCardDescriptorMatches(line, card) {
  ...
  if (/noncreature spell/.test(text) && /creature/.test(type)) return false;
  if (/creature spell/.test(text) && !/creature/.test(type)) return false;
  if (/artifact spell/.test(text) && !/artifact/.test(type)) return false;
  if (/enchantment spell/.test(text) && !/enchantment/.test(type)) return false;
  if (/instant(?: or sorcery)? spell/.test(text) && !(/instant/.test(type) || (/or sorcery/.test(text) && /sorcery/.test(type)))) return false;
  if (/sorcery(?: or instant)? spell/.test(text) && !(/sorcery/.test(type) || (/or instant/.test(text) && /instant/.test(type)))) return false;
  return true;
}
...
function triggerPerspectiveMatches(line, sourceCard, event) {
  ...
  if (/\byou\b/.test(text) && /\b(?:cast|draw|gain|lose|attack|activate|mill)\b/.test(text) && !/\bopponent\b/.test(text)) {
    if (actor && controller !== actor) return false;
  }
  if (/\b(?:an |each )?opponent\b/.test(text) && actor && controller === actor) return false;
  return true;
}

function lineMatchesGameEvent(line, sourceCard, event) {
  const text = String(line || '').toLowerCase();
  if (!/\bwhen(?:ever)?\b/.test(text)) return false;
  if (!triggerPerspectiveMatches(line, sourceCard, event)) return false;
  if (event.type === 'cast') {
    if (!/\bcast(?:s|ing)?\b/.test(text)) return false;
    if (/when you cast this spell/.test(text) && sourceCard.instanceId !== event.card?.instanceId) return false;
    return triggerCardDescriptorMatches(line, event.card);
  }
  if (event.type === 'draw') return /\bdraws? (?:a|one|two|three|\d+)?\s*cards?\b|\bdraw a card\b/.test(text);
  if (event.type === 'life-gain') return /\bgain(?:s)? (?:\d+|a|life)|\bgain life\b/.test(text);
  if (event.type === 'life-loss') return /\bloses? (?:\d+|a|life)|\blose life\b/.test(text);
  if (event.type === 'attack') {
    if (!/\battack(?:s|ed)?\b/.test(text)) return false;
    if (/\bthis creature attacks\b/.test(text)) return sourceCard.instanceId === event.card?.instanceId;
    return true;
  }
  if (event.type === 'ability-activated') return /\bactivate(?:s|d)? (?:an|a|one or more).*abilit/.test(text);
  if (event.type === 'counter-added') {
    if (!/\bcounter(?:s)? (?:is|are|were|was)? ?put\b|\bput .*counter/.test(text)) return false;
  ...
  if (event.type === 'tapped') {
    ...
    return cardMatchesEventSubject(event.card, line);
  }
  if (event.type === 'mill') return /\bmill(?:s|ed)?\b|put into .*graveyard from .*library/.test(text);
  if (event.type === 'control-change') {
    if (!/\bgain(?:s|ed)? control\b/.test(text)) return false;
...
function emitGameEvent(draft, event) {
  ...
  if (event.card && !sources.some((card) => card.instanceId === event.card.instanceId)) sources.push(event.card);
...
function enqueueBattlefieldEntryEffects(draft, enteringCard, originalZone) {
  ...
  for (const sourceCard of allPermanents) {
    for (const line of oracleAbilityLines(sourceCard)) {
      ...
      const underControl = triggerSubjectControlMatches(line, sourceCard, enteringCard);
  ...
  for (const sourceCard of allSources) {
    for (const line of oracleAbilityLines(sourceCard)) {
      ...
      const exileTrigger = destinationZone === 'exile'
        ...
        && (ownCard || subjectMatches);
      if ((diesTrigger || graveyardTrigger || leavesTrigger || exileTrigger) && triggerSubjectControlMatches(line, sourceCard, leavingCard)) {
        queuePendingEffect(draft, {
          sourceCard,
          controllerId: sourceCard.controller,
...
function moveCard(instanceId, targetPlayerId, targetZone, { force = false, libraryPosition = 'top', enterTapped = false, countsAsLandPlay = true, xValue = null, auraTargetId = null, skipPayment = false, testOverride = false } = {}) {
  ...
  if (!legality.legal && !force) {
    ...
    const label = currentState.settings.testOverrides ? 'Use Test Override for this move?' : 'Use a manual rules override for this move?';
...
Press Cancel to leave ${sourceIsMerged ? 'the commander component' : 'it'} in ${ZONE_LABELS[targetZone]}.`);
  ...
  }, { log: `${testOverride ? '[TEST OVERRIDE] ' : ''}${source.card.name}: ${ZONE_LABELS[source.zone]} → ${ZONE_LABELS[targetZone]}.${skipPayment && castAttempt ? ' Cost intentionally bypassed.' : autoManaText}${battlefieldEntryText}` });
  return { ok: true, message: testOverride ? 'Test override applied.' : (autoManaText ? autoManaText.trim() : 'Card moved.') };
}
...
function toggleAttack(instanceId) {
  ...
  updateState((draft) => {
    ...
    if (located.card.attacking) {
      const becameTapped = !located.card.tapped;
      ...
      emitGameEvent(draft, { type: 'attack', actorId: located.card.controller, card: located.card });
      recordPublicEvent(draft, {
        type: 'attack',
...
function createToken(playerId, token, options = {}) {
  ...
  updateState((draft) => {
    for (const card of cards) {
      draft.players[playerId].zones.battlefield.push(card);
      enqueueBattlefieldEntryEffects(draft, card, 'token-created');
    }
  }, {
...
function resolveStackTop() {
  ...
  if (card._specialAbility === 'ninjutsu') {
    updateState((draft) => { const ability = draft.stack.pop(); const source = findCard(ability.sourceCardId, draft); if (source && source.zone === 'hand') { const ninja = source.container.splice(source.index, 1)[0]; ninja.controller = ability.controller; ninja.tapped = true; ninja.attacking = true; ninja.blocking = null; ninja.blockedBy = []; ninja.summoningSick = true; ninja.enteredBattlefieldTurn = Number(draft.turnNumber || 1); draft.players[ability.controller].zones.battlefield.push(ninja); enqueueBattlefieldEntryEffects(draft, ninja, 'hand'); } draft.priorityPlayerId = draft.activePlayerId; draft.consecutivePasses = 0; }, { log: `${card.sourceCardName || card.name} entered the battlefield tapped and attacking via ninjutsu.` });
    return { ok: true };
  }
...
function counterStackTop() {
  ...
  updateState((draft) => {
    ...
    if (countered?._specialAbility === 'ninjutsu') { draft.priorityPlayerId = draft.activePlayerId; draft.consecutivePasses = 0; return; }
...
function activateBattlefieldAbility(instanceId, abilityIndex = 0) {
  ...
  updateState((draft) => {
    ...
    emitGameEvent(draft, { type: 'ability-activated', actorId: located.card.controller, card: located.card });
...
const { buildPlayerDeck, canUndo, createInitialState, drawCards, findCard, getState, importState, resetState, restore, setState, subscribe, undo, updateState } = __modules["./state.js"];
...
const MULTIPLAYER_APP_VERSION = '5.35.1';
const multiplayer = {
  mode: 'solo',
  role: null,
  localPlayerId: null,
...
function requestUndoApproval() {
  ...
  if (!isMultiplayerConnected()) return toast('The other player must be connected to approve an undo.', true);
  if (ui.undoRequest) return toast('An undo request is already pending.', true);
  const requestId = uid('undo-request');
  multiplayer.undoRequestId = requestId;
  ui.undoRequest = {
    id: requestId,
    direction: 'outgoing',
    requesterId: multiplayer.localPlayerId,
    requesterName: getState().players[multiplayer.localPlayerId]?.name || 'Other player',
    status: 'waiting',
  };
  sendNetwork({ type: 'undo-request', requestId, requesterId: multiplayer.localPlayerId, requesterName: ui.undoRequest.requesterName });
  render();
}

function approveUndoRequest() {
  const request = ui.undoRequest;
  if (!request || request.direction !== 'incoming') return;
  sendNetwork({ type: 'undo-response', requestId: request.id, approved: true });
  ui.undoRequest = null;
  render();
  toast('Undo approved. Waiting for the other player to restore the prior state.');
}

function denyUndoRequest() {
  const request = ui.undoRequest;
  if (!request || request.direction !== 'incoming') return;
  sendNetwork({ type: 'undo-response', requestId: request.id, approved: false });
  ui.undoRequest = null;
  render();
  toast('Undo request denied.');
}

function cancelUndoRequest() {
  const request = ui.undoRequest;
  if (!request || request.direction !== 'outgoing') return;
  sendNetwork({ type: 'undo-cancel', requestId: request.id });
  ui.undoRequest = null;
  multiplayer.undoRequestId = '';
  render();
}
...
function requestOverrideApproval(payload = {}) {
  if (!isOnlineMultiplayer()) return handleResult(applyApprovedOverride(payload));
  if (!isMultiplayerConnected()) return toast('The other player must be connected to approve a Test Override.', true);
  if (ui.undoRequest || ui.overrideRequest) return toast('Another multiplayer approval request is already pending.', true);
  const requestId = uid('override-request');
  multiplayer.overrideRequestId = requestId;
  ui.overrideRequest = {
    id: requestId,
    direction: 'outgoing',
    requesterId: multiplayer.localPlayerId,
    ...
    description: safeOverrideDescription(payload),
  };
  sendNetwork({
    type: 'override-request',
    requestId,
    requesterId: multiplayer.localPlayerId,
    requesterName: ui.overrideRequest.requesterName,
    description: ui.overrideRequest.description,
    payload: deepClone(payload),
  });
  render();
}

function approveOverrideRequest() {
  const request = ui.overrideRequest;
  if (!request || request.direction !== 'incoming') return;
  sendNetwork({ type: 'override-response', requestId: request.id, approved: true });
  ui.overrideRequest = null;
  render();
  toast('Test Override approved. Waiting for the other player to apply it.');
}

function denyOverrideRequest() {
  const request = ui.overrideRequest;
  if (!request || request.direction !== 'incoming') return;
  sendNetwork({ type: 'override-response', requestId: request.id, approved: false });
  ui.overrideRequest = null;
  render();
  toast('Test Override denied.');
}

function cancelOverrideRequest() {
  const request = ui.overrideRequest;
  if (!request || request.direction !== 'outgoing') return;
  sendNetwork({ type: 'override-cancel', requestId: request.id });
  ui.overrideRequest = null;
  multiplayer.overrideRequestId = '';
  render();
}
...
function handleNetworkMessage(message) {
  ...
  if (message.type === 'undo-request') {
    if (!ui.undoRequest) {
      ui.undoRequest = { id: message.requestId, direction: 'incoming', requesterId: message.requesterId, requesterName: message.requesterName || 'Other player', status: 'waiting' };
      render();
      toast(`${message.requesterName || 'The other player'} requested an undo.`);
    } else {
      sendNetwork({ type: 'undo-response', requestId: message.requestId, approved: false, reason: 'another-request-pending' });
    }
  }
  if (message.type === 'undo-response' && ui.undoRequest?.direction === 'outgoing' && message.requestId === ui.undoRequest.id) {
    const approved = Boolean(message.approved);
    ui.undoRequest = null;
    multiplayer.undoRequestId = '';
    if (approved) {
      const ok = undo();
      if (ok) toast('Undo approved and applied.');
      else toast('The prior state was no longer available.', true);
    } else {
      render();
      toast(message.reason === 'another-request-pending' ? 'The other player already has an undo request open.' : 'The other player denied the undo.', true);
    }
  }
  if (message.type === 'undo-cancel' && ui.undoRequest?.direction === 'incoming' && message.requestId === ui.undoRequest.id) {
    ui.undoRequest = null;
    render();
    toast('Undo request was cancelled.');
  }
  if (message.type === 'override-request') {
    if (!ui.undoRequest && !ui.overrideRequest) {
      ui.overrideRequest = {
        id: message.requestId,
        direction: 'incoming',
        requesterId: message.requesterId,
        requesterName: message.requesterName || 'Other player',
        status: 'waiting',
        description: message.description || `${message.requesterName || 'The other player'} wants to use Test Override.`,
        payload: message.payload || {},
      };
      render();
      toast(`${message.requesterName || 'The other player'} requested a Test Override.`);
    } else {
      sendNetwork({ type: 'override-response', requestId: message.requestId, approved: false, reason: 'another-request-pending' });
    }
  }
  if (message.type === 'override-response' && ui.overrideRequest?.direction === 'outgoing' && message.requestId === ui.overrideRequest.id) {
    const request = ui.overrideRequest;
    const approved = Boolean(message.approved);
    ui.overrideRequest = null;
    multiplayer.overrideRequestId = '';
    if (approved) {
      const result = applyApprovedOverride(request.payload || {});
      handleResult(result);
      if (result?.ok) toast('Test Override approved and applied.');
    } else {
      render();
      toast(message.reason === 'another-request-pending' ? 'The other player already has an approval request open.' : 'The other player denied the Test Override.', true);
    }
  }
  if (message.type === 'override-cancel' && ui.overrideRequest?.direction === 'incoming' && message.requestId === ui.overrideRequest.id) {
    ui.overrideRequest = null;
    render();
    toast('Test Override request was cancelled.');
  }
...
function renderUndoApproval() {
  ...
  if (request.direction === 'incoming') {
    return `<div class="forge-approval-backdrop"><section class="forge-approval-card" role="dialog" aria-modal="true" aria-labelledby="forge-undo-request-title"><div class="forge-approval-kicker">MULTIPLAYER APPROVAL</div><div class="forge-approval-icon">↶</div><h2 id="forge-undo-request-title">${escapeHtml(request.requesterName || 'Other player')} wants to undo</h2><div class="forge-approval-detail"><span>Action to rewind</span><strong>${escapeHtml(lastAction)}</strong></div><p>This restores the prior saved game state only if you approve it. Nothing changes while this request is waiting.</p><div class="forge-approval-actions"><button class="btn primary" data-action="approve-undo">✓ Allow undo</button><button class="btn danger" data-action="deny-undo">✕ Deny</button></div></section></div>`;
  }
  return `<div class="forge-approval-waiting"><div><strong>↶ Undo requested</strong><span>Waiting for the other player to approve…</span></div><button class="btn small-btn ghost" data-action="cancel-undo">Cancel request</button></div>`;
}
...
function renderOverrideApproval() {
  ...
  if (!request) return '';
  if (request.direction === 'incoming') {
    return `<div class="forge-approval-backdrop"><section class="forge-approval-card override" role="dialog" aria-modal="true" aria-labelledby="forge-override-request-title"><div class="forge-approval-kicker">TEST OVERRIDE REQUEST</div><div class="forge-approval-icon">🧪</div><h2 id="forge-override-request-title">${escapeHtml(request.requesterName || 'Other player')} wants to override the rules</h2><div class="forge-approval-detail"><span>Requested test action</span><strong>${escapeHtml(request.description || 'Use a Commander Forge Test Override.')}</strong></div><p>Test Override can intentionally bypass normal mana, timing, or zone restrictions. The action is applied only if you allow it.</p><div class="forge-approval-actions"><button class="btn primary" data-action="approve-override">✓ Allow override</button><button class="btn danger" data-action="deny-override">✕ Deny</button></div></section></div>`;
  }
  return `<div class="forge-approval-waiting override"><div><strong>🧪 Override requested</strong><span>Waiting for the other player to approve…</span></div><button class="btn small-btn ghost" data-action="cancel-override">Cancel request</button></div>`;
}
...
function renderSmartEffectActions(effect, state) {
  ...
  if (smart.kind === 'sacrifice-creature') {
    ...
    if (!smart.cards.length) return `<div class="mana-choice-group"><span class="small muted wide">${escapeHtml(victim?.name || 'Affected player')} controls no creatures to sacrifice.</span><button class="btn small-btn ghost" data-action="smart-effect-no-result" data-effect-id="${effect.id}">Resolve — nothing to sacrifice</button><button class="btn small-btn ghost" data-action="resolve-effect" data-effect-id="${effect.id}">Override — mark resolved</button></div>`;
    return `<div class="mana-choice-group"><div class="small muted wide">${escapeHtml(victim?.name || 'Affected player')} chooses a creature to sacrifice</div>${smart.cards.slice(0, 20).map((card) => `<button class="btn small-btn danger" data-action="smart-effect-card" data-effect-id="${effect.id}" data-card-id="${card.instanceId}">Sacrifice ${escapeHtml(card.name)}</button>`).join('')}<button class="btn small-btn ghost" data-action="resolve-effect" data-effect-id="${effect.id}">Override — mark resolved</button></div>`;
  }
  const simpleLabels = {
...
function renderTestOverrideActions(card, selected, state) {
  ...
  return `<div class="inspector-section test-override-section"><h3>🧪 Test Override</h3><div class="validation warn"><strong>Sandbox controls</strong><br />These intentionally bypass mana, timing, land-play, and normal zone restrictions so you can build a board state or test a combo. They are not a claim that the move is legal in Magic.</div><div class="action-grid">${battlefieldButtons}<button class="btn ghost" data-action="test-force-move" data-card-id="${card.instanceId}" data-target-player-id="${ownerId}" data-zone="hand">Force owner hand</button><button class="btn ghost" data-action="test-force-move" data-card-id="${card.instanceId}" data-target-player-id="${ownerId}" data-zone="graveyard">Force owner graveyard</button><button class="btn ghost" data-action="test-force-move" data-card-id="${card.instanceId}" data-target-player-id="${ownerId}" data-zone="exile">Force owner exile</button><button class="btn ghost" data-action="test-force-move" data-card-id="${card.instanceId}" data-target-player-id="${controllerId}" data-zone="stack">Force stack / no cost</button><button class="btn ghost" data-action="test-force-move" data-card-id="${card.instanceId}" data-target-player-id="${ownerId}" data-zone="library" data-position="top">Force library top</button><button class="btn ghost" data-action="test-force-move" data-card-id="${card.instanceId}" data-target-player-id="${ownerId}" data-zone="library" data-position="bottom">Force library bottom</button></div><p class="small muted">In online play, Test Override requests use a themed in-game approval popup and do nothing until the other player allows them. Undo uses the same approval style.</p></div>`;
}
  ...
  try {
    ...
    if (action === 'toggle-smart-effect-multi') {
      ...
      if (selected.has(cardId)) selected.delete(cardId);
        ...
        if (card && (card._smartPower === null || card._smartPower === undefined)) {
