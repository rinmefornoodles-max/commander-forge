...
function pendingEffectMoveAuthorization(state, card, fromZone, targetZone, targetPlayerId, libraryPosition = 'top') {
  ...
  for (const effect of state.pendingTriggers || []) {
    ...
    if (fromZone === 'graveyard') {
      ...
      if (!desc || desc.maxCount !== 1 || desc.destination !== actualDestination) continue;
      const opponentId = otherPlayerId(state, effect.controllerId);
      if (desc.graveyardScope === 'controller' && card.owner !== effect.controllerId) continue;
      if (desc.graveyardScope === 'opponent' && card.owner !== opponentId) continue;
...
const MULTIPLAYER_APP_VERSION = '6.0.0-alpha.6.1-mp6';
const multiplayer = {
  mode: 'solo',
  role: null,
  localPlayerId: null,
...
function multiplayerStatusLabel() {
  ...
  if (multiplayer.status === 'connected' || multiplayer.status === 'waiting') {
    ...
    return `${multiplayer.localPlayerId ? `Player ${String(multiplayer.localPlayerId).replace(/^p/, '')}` : 'Joining'} · ${connected}/${seats} · ${multiplayer.roomCode}`;
  }
...
function attachHostGuestConnection(connection) {
  if (multiplayer.role !== 'host') { try { connection.close(); } catch {} return; }
  ...
  connection.on('open', () => {
    ...
    multiplayer.connectedPlayerIds = ['p1', ...Object.keys(multiplayer.connections).filter((id) => multiplayer.connections[id]?.open)];
    try {
      connection.send({
  ...
  connection.on('data', (message) => handleNetworkMessage(message, playerId));
  connection.on('close', () => {
...
function attachGuestConnection(connection) {
  ...
  connection.on('open', () => {
    multiplayer.status = 'connected';
    multiplayer.lastError = '';
    try { connection.send({ version: MULTIPLAYER_APP_VERSION, type: 'hello', role: 'guest' }); } catch {}
    render();
  });
  connection.on('data', (message) => handleNetworkMessage(message, 'p1'));
  connection.on('close', () => {
...
function createOnlineHost(playerCountValue = null) {
  ...
  const createPeer = () => {
    ...
    multiplayer.peer.on('open', () => { multiplayer.status = 'waiting'; broadcastLobbyState(); render(); });
    multiplayer.peer.on('connection', attachHostGuestConnection);
...
function submitStartingRoll(result) {
  const playerId = multiplayer.localPlayerId;
  if (!playerId || !getState().startingRoll?.active) return;
  if (multiplayer.role === 'host') {
...
function handleNetworkMessage(message, senderPlayerId = null) {
  ...
  if (message.type === 'hello' && multiplayer.role === 'host') {
    broadcastLobbyState();
    return;
  }
  ...
  if (message.type === 'state' && multiplayer.role === 'guest') { applyNetworkState(message.state, message.revision); return; }
  if (message.type === 'state-proposal' && multiplayer.role === 'host') {
    if (Number(message.baseRevision || 0) !== multiplayer.revision) {
      sendNetwork({ type: 'state', revision: multiplayer.revision, state: stateForNetwork() }, senderPlayerId);
      return;
    }
  ...
  // Approval requests remain intentionally simple in 3+ player games: the host
  // is the table authority and receives the request. This avoids multiple modal
  // races while the new multiplayer lobby is being introduced.
  if (message.type === 'undo-request' || message.type === 'override-request') {
    if (multiplayer.role === 'host') {
      toast(`${message.requesterName || 'A player'} requested ${message.type === 'undo-request' ? 'an undo' : 'a Test Override'}. Use the host table controls to handle it.`, true);
    }
  }
}
...
function renderOnlineSetup(state) {
  ...
  if (!localId) return `${connectionPanel}<div class="validation multiplayer-note">Waiting for the host to assign your seat…</div>`;
  ensureMultiplayerSeatUi();
...
function renderTable(state) {
  const localId = isOnlineMultiplayer() && multiplayer.localPlayerId ? multiplayer.localPlayerId : 'p1';
  const ids = (state.turnOrder || Object.keys(state.players || {})).filter((id)=>state.players[id]);
  if (ids.length <= 2) {
...
function renderStartingRollModal(state) {
  ...
  return `<div class="modal-backdrop starting-roll-backdrop"><section class="modal starting-roll-modal" role="dialog" aria-modal="true" aria-labelledby="starting-roll-title"><header class="starting-roll-header"><div><div class="mulligan-kicker">WHO GOES FIRST?</div><h2 id="starting-roll-title">Throw the D20</h2><p>${escapeHtml(roll.message || 'Highest roll goes first.')}</p></div><span class="d20-round">Round ${Number(roll.round || 1)}</span></header><div class="starting-roll-body"><div class="d20-results-grid">${results}</div><div class="d20-arena-wrap"><canvas id="forge-d20-canvas" data-can-roll="${canRoll ? 'true' : 'false'}"></canvas>${canRoll ? '<div class="d20-instruction"><strong>Click and hold the die, drag, then release to throw.</strong><span>Throw speed is capped so the die stays on the table. It has momentum, friction, and bounces off the arena edges.</span></div>' : `<div class="d20-instruction waiting"><strong>${localResult ? `You rolled ${localResult}.` : required.has(localId) ? 'Waiting for your roll…' : 'You are not in this tie reroll.'}</strong><span>Waiting for the remaining players.</span></div>`}</div></div></section></div>`;
}
...
function renderSettingsModal(state) {
  const v6 = window.CommanderForgeEngine6?.getStatus?.() || { state: 'starting', version: '6.0.0-alpha.6.1' };
  const engineStatus = v6.state === 'ready'
    ? `<div class="validation ok"><strong>Engine 6 ready.</strong> ${escapeHtml(v6.version || '6.0.0-alpha.6.1')} · Browser Web Worker${v6.lastSyncAt ? ' · table mirrored' : ''}${v6.lastCompile?.cards != null ? ` · ${v6.lastCompile.cards} card definitions analyzed${v6.lastCompile.unsupportedClauses ? ` · ${v6.lastCompile.unsupportedClauses} unsupported Oracle clause(s) kept for fallback` : ''}` : ''}</div>`
    : v6.state === 'error'
