...
function renderCommanderZoneCard(card, state, playerId) {
  ...
  const currentCost = `${baseCost}${tax ? ` + {${tax}}` : ''}`;
  return `<div class="commander-zone-card">
    <div class="commander-zone-card-main">${renderCard(card, state, { compact: true })}</div>
    <div class="commander-tax-panel">
      <span><strong>Tax</strong> +${tax}</span>
      <span><strong>Cast</strong> ${escapeHtml(currentCost)}</span>
      <span class="muted">Cast ${casts} time${casts === 1 ? '' : 's'} from command</span>
    </div>
    <button class="btn small-btn commander-cast-btn" data-action="cast-commander" data-card-id="${card.instanceId}" data-player-id="${playerId}">Cast commander</button>
  </div>`;
}
...
app.addEventListener('click', async (event) => {
  ...
  try {
    ...
    if (action === 'cast-commander') handleResult(moveCard(button.dataset.cardId, button.dataset.playerId, 'battlefield'));
