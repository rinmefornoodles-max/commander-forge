...
const MULTIPLAYER_APP_VERSION = '6.0.0-alpha.6.3-mp6';
const multiplayer = {
  mode: 'solo',
  role: null,
  localPlayerId: null,
...
function attachGuestConnection(connection) {
  ...
  const onGuestConnectionOpen = () => {
    if (guestOpened || !connection.open) return;
    guestOpened = true;
    multiplayer.status = 'connected';
    multiplayer.lastError = '';
    multiplayer.seatAssignmentAttempts = 0;
  ...
  connection.on('open', onGuestConnectionOpen);
...
function renderOnlineSetup(state) {
  ...
  if (!localId) return `${connectionPanel}<div class="validation multiplayer-note"><strong>Waiting for the host to assign your seat…</strong><span>The request retries automatically. If it takes more than a moment, you can retry it manually.</span><button class="btn small-btn" data-action="request-seat-assignment">Request seat again</button></div>`;
  ensureMultiplayerSeatUi();
...
function renderSettingsModal(state) {
  const v6 = window.CommanderForgeEngine6?.getStatus?.() || { state: 'starting', version: '6.0.0-alpha.6.3' };
  const engineStatus = v6.state === 'ready'
    ? `<div class="validation ok"><strong>Engine 6 ready.</strong> ${escapeHtml(v6.version || '6.0.0-alpha.6.3')} · Browser Web Worker${v6.lastSyncAt ? ' · table mirrored' : ''}${v6.lastCompile?.cards != null ? ` · ${v6.lastCompile.cards} card definitions analyzed${v6.lastCompile.unsupportedClauses ? ` · ${v6.lastCompile.unsupportedClauses} unsupported Oracle clause(s) kept for fallback` : ''}` : ''}</div>`
    : v6.state === 'error'
