...
function renderHiddenHand(count) {
  return Array.from({ length: Math.min(count, 12) }, (_, index) => `<div class="game-card hidden-hand-card" aria-label="Hidden card ${index + 1}"><img src="./assets/card-back.svg" alt="Card back" /></div>`).join('');
}
...
function renderCard(card, state, { compact = false } = {}) {
  ...
  const image = card.faceDown ? './assets/card-back.svg' : cardSmallImage(card);
  return `<article class="game-card ${selected ? 'selected' : ''} ${card.tapped ? 'tapped' : ''} ${card.attacking ? 'attacking' : ''} ${compact ? 'compact' : ''}" data-card-id="${card.instanceId}" title="${escapeHtml(card.name)}"><img src="${escapeHtml(image)}" alt="${escapeHtml(card.name)}" draggable="false" onerror="this.src='./assets/card-back.svg'" /><div class="badge-row">${badges.join('')}</div>${state.settings.showCardNames ? `<div class="card-name-strip">${escapeHtml(card.name)}</div>` : ''}</article>`;
}
...
function renderInspector(state) {
  ...
  return `
    ...
    <div class="inspector-section">
      <img class="inspector-card-image" src="${escapeHtml(cardImage(card))}" alt="${escapeHtml(card.name)}" onerror="this.src='./assets/card-back.svg'" />
...
function renderLibraryBacks(count) {
  ...
  return `${Array.from({ length: visible }, () => `<div class="game-card card-back-stack"><img src="./assets/card-back.svg" alt="Hidden library card" /></div>`).join('')}<div class="validation"><strong>Library is hidden.</strong><br />Use search when a tutor effect lets you find a card, or reveal the top card.</div>`;
}
...
function demoCard(name, owner, index) {
  ...
  return {
    ...
    image: './assets/demo-card.svg', imageSmall: './assets/demo-card.svg', owner, controller: owner,
