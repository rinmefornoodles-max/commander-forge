...
function compiledStaticAbilitiesForPermanent(permanent) {
  const compiler = globalThis.CommanderForgeOracleCompilerV7;
  if (!compiler?.compileStaticAbilities || !permanent?.oracleText) return [];
  try {
    return compiler.compileStaticAbilities(permanent.oracleText, {
      cardName: permanent.name,
    }).staticAbilities || [];
  } catch (error) {
    console.warn('[Commander Forge static rules]', permanent?.name, error);
    return [];
  }
}

function staticRestrictionAppliesToPlayer(sourcePermanent, actorId, appliesTo) {
  const sourceController = sourcePermanent?.controller || sourcePermanent?.owner;
  if (!actorId || !sourceController) return false;

  if (appliesTo === 'OPPONENTS') return actorId !== sourceController;
  if (appliesTo === 'ALL_PLAYERS') return true;
  if (appliesTo === 'YOU') return actorId === sourceController;
  return false;
}

function effectiveSpellManaValue(card, options = {}) {
  const base = Number(card?.manaValue ?? card?.cmc ?? 0);
  const chosenX = Math.max(0, Number(options.xValue ?? card?.castXValue ?? 0));
  const xSymbols = (String(card?.manaCost || '').match(/\{X\}/gi) || []).length;
  return Math.max(0, base + chosenX * xSymbols);
}

function staticPredicateMatchesCard(card, predicate, options = {}) {
  if (!predicate || predicate.property === 'ANY') return true;

  const manaValue = options.spell
    ? effectiveSpellManaValue(card, options)
    : Math.max(0, Number(card?.manaValue ?? card?.cmc ?? 0));

  if (predicate.property === 'MANA_VALUE_PARITY') {
    const even = manaValue % 2 === 0;
    return predicate.parity === 'EVEN' ? even : !even;
  }

  if (predicate.property === 'MANA_VALUE_COMPARE') {
    const target = Number(predicate.value || 0);
    if (predicate.operator === 'GTE') return manaValue >= target;
    if (predicate.operator === 'LTE') return manaValue <= target;
    if (predicate.operator === 'GT') return manaValue > target;
    if (predicate.operator === 'LT') return manaValue < target;
    if (predicate.operator === 'EQ') return manaValue === target;
  }

  return false;
}

function staticRestrictionCardTypeMatches(card, cardType = 'ANY') {
  const typeLine = String(card?.typeLine || '').toLocaleLowerCase();
  const type = String(cardType || 'ANY').toLocaleUpperCase();
  if (type === 'ANY') return true;
  if (type === 'CREATURE') return typeLine.includes('creature');
  if (type === 'NONCREATURE') return !typeLine.includes('creature');
  return true;
}

function compiledCastRestrictionViolations(state, playerId, card, sourceZone = 'hand', options = {}) {
  const reasons = [];

  for (const player of Object.values(state.players || {})) {
    for (const permanent of player.zones?.battlefield || []) {
      for (const rule of compiledStaticAbilitiesForPermanent(permanent)) {
        if (!['CAST_PROHIBITION', 'CAST_LIMIT'].includes(rule.type)) continue;
        if (!staticRestrictionAppliesToPlayer(permanent, playerId, rule.appliesTo)) continue;
        if (!staticRestrictionCardTypeMatches(card, rule.cardType)) continue;

        if (rule.sourceZone && String(sourceZone || '').toLocaleLowerCase() !== String(rule.sourceZone).toLocaleLowerCase()) continue;
        if (rule.timing === 'SOURCE_CONTROLLER_TURN' && state.activePlayerId !== (permanent.controller || permanent.owner)) continue;

        if (rule.type === 'CAST_LIMIT') {
          if (Number(state.players[playerId]?.spellsCastThisTurn || 0) >= Number(rule.limitPerTurn || 1)) {
            reasons.push(`${permanent.name} limits ${state.players[playerId]?.name || 'that player'} to ${rule.limitPerTurn || 1} spell each turn.`);
          }
          continue;
        }

        if (!staticPredicateMatchesCard(card, rule.predicate, { ...options, spell: true })) continue;

        const manaValue = effectiveSpellManaValue(card, options);
        if (rule.predicate?.property === 'MANA_VALUE_PARITY') {
          reasons.push(`${permanent.name} prevents ${state.players[playerId]?.name || 'that player'} from casting ${card.name} because its mana value ${manaValue} is ${String(rule.predicate.parity || '').toLocaleLowerCase()}.`);
        } else {
          reasons.push(`${permanent.name} prevents ${state.players[playerId]?.name || 'that player'} from casting ${card.name}.`);
        }
      }
    }
  }
...
function compiledBlockRestrictionViolations(state, blockerPlayerId, blocker) {
  const reasons = [];

  for (const player of Object.values(state.players || {})) {
    for (const permanent of player.zones?.battlefield || []) {
      for (const rule of compiledStaticAbilitiesForPermanent(permanent)) {
        if (rule.type !== 'BLOCK_PROHIBITION') continue;
        if (!staticRestrictionAppliesToPlayer(permanent, blockerPlayerId, rule.appliesTo)) continue;
        if (!staticPredicateMatchesCard(blocker, rule.predicate, { spell: false })) continue;

        const manaValue = Math.max(0, Number(blocker?.manaValue ?? blocker?.cmc ?? 0));
        if (rule.predicate?.property === 'MANA_VALUE_PARITY') {
          reasons.push(`${permanent.name} prevents ${blocker.name} from blocking because its mana value ${manaValue} is ${String(rule.predicate.parity || '').toLocaleLowerCase()}.`);
        } else {
          reasons.push(`${permanent.name} prevents ${blocker.name} from blocking.`);
        }
      }
    }
  }
...
function compiledBlockLegality(state, blockerPlayerId, blocker) {
  const reasons = compiledBlockRestrictionViolations(state, blockerPlayerId, blocker);
  return { legal: reasons.length === 0, reasons };
}

function battlefieldCastingRestrictions(state, playerId, card, sourceZone = 'hand', options = {}) {
  const reasons = [
    ...compiledCastRestrictionViolations(state, playerId, card, sourceZone, options),
  ];
  ...
  for (const permanent of allPermanents) {
    ...
    // Keep legacy coverage for rules not yet migrated into the compiler.
    ...
    // These two families are now also compiled. Keep the legacy branch only
    // when the compiler did not emit the equivalent static descriptor.
    const compiled = compiledStaticAbilitiesForPermanent(permanent);
    const hasCompiledOpponentTurnLock = compiled.some((rule) =>
      rule.type === 'CAST_PROHIBITION' && rule.timing === 'SOURCE_CONTROLLER_TURN'
    );
    const hasCompiledGraveyardLock = compiled.some((rule) =>
      rule.type === 'CAST_PROHIBITION' && rule.sourceZone === 'graveyard'
    );

    if (!hasCompiledOpponentTurnLock && /your opponents can't cast spells during your turn/.test(text) && permanent.controller !== playerId && state.activePlayerId === permanent.controller) {
      reasons.push(`${permanent.name} prevents opponents from casting spells during its controller's turn.`);
    }

    if (!hasCompiledGraveyardLock && /players can't cast spells from graveyards/.test(text) && sourceZone === 'graveyard') {
      reasons.push(`${permanent.name} prevents casting from graveyards.`);
    }
  }
...
function spellCastLegality(state, playerId, card, sourceZone = 'hand', options = {}) {
  ...
  reasons.push(...battlefieldCastingRestrictions(state, playerId, card, sourceZone, options));
...
function attackTaxDescriptorsForPermanent(card) {
  const compiler = globalThis.CommanderForgeOracleCompilerV7;
  if (!compiler?.parseAttackCostStatics || !card?.oracleText) return [];
  try {
    return compiler.parseAttackCostStatics(card.oracleText);
  } catch (error) {
    console.warn('[Commander Forge attack tax parser]', card?.name, error);
    return [];
  }
}

function resolvedAttackTaxCost(state, defenderId, permanent, descriptor) {
  let cost = String(descriptor?.perAttackerCost || '').trim();
  if (!cost) return { supported: false, cost: '', manaValue: 0, reason: 'Attack tax has no readable mana cost.' };

  if (descriptor?.sourceMustBeUntapped && permanent.tapped) {
    return { supported: true, inactive: true, cost: '', manaValue: 0 };
  }

  if (descriptor?.dynamicAmount === 'ENCHANTMENTS_YOU_CONTROL') {
    const enchantments = (state.players?.[defenderId]?.zones?.battlefield || [])
      .filter((card) => /\bEnchantment\b/i.test(String(card.typeLine || '')))
      .length;
    cost = cost.replace(/\{X\}/gi, `{${Math.max(0, enchantments)}}`);
  }

  // Current mana planner supports ordinary colored/colorless/generic/hybrid-like
  // symbols it already understands. Phyrexian/life alternatives are left
  // conservative rather than silently treated as free.
  if (/\/P\}/i.test(cost)) {
    return {
      supported: false,
      cost,
      manaValue: 0,
      reason: `${permanent.name} uses a Phyrexian/life attack payment that Forge does not yet auto-pay.`,
    };
  }

  const req = manaRequirement(cost, 0);
  const manaValue = Number(req.generic || 0)
    + ['W', 'U', 'B', 'R', 'G', 'C'].reduce((sum, color) => sum + Number(req[color] || 0), 0)
    + Number(req.flexible?.length || 0);

  return { supported: true, inactive: false, cost, manaValue };
}

function attackTaxSources(state, attackerId, defenderId) {
  if (!state.players?.[attackerId] || !state.players?.[defenderId]) return [];

  const results = [];

  for (const permanent of state.players[defenderId].zones?.battlefield || []) {
    for (const descriptor of attackTaxDescriptorsForPermanent(permanent)) {
      const resolved = resolvedAttackTaxCost(state, defenderId, permanent, descriptor);
      if (resolved.inactive) continue;
...
function repeatManaCost(cost, count) {
  return Array.from({ length: Math.max(0, Number(count || 0)) }, () => String(cost || '')).join('');
}

function attackTaxPlan(state, attackerId, defenderId, attackerCount = 1, options = {}) {
  const count = Math.max(0, Number(attackerCount || 0));
  const sources = attackTaxSources(state, attackerId, defenderId);
  ...
  if (!count || !sources.length) {
    return {
      ...
      manaCost: '',
      manaValue: 0,
      payment: { ok: true, sources: [], finalManaCost: '' },
    };
  }

  const unsupported = sources.find((source) => !source.supported);
  if (unsupported) {
    return {
      ok: false,
      required: true,
      ...
      defenderId,
      sources,
      manaCost: unsupported.cost || '',
      manaValue: 0,
      reason: unsupported.reason,
      payment: { ok: false, sources: [] },
    };
  }

  const manaCost = sources.map((source) => repeatManaCost(source.cost, count)).join('');
  const manaValue = sources.reduce((sum, source) => sum + Number(source.manaValue || 0) * count, 0);
  const player = state.players[attackerId];

  let payment;

  if (options.useSources === false) {
    const canPay = canPayMana(player?.mana || {}, manaCost, 0);
    payment = {
      ...canPay,
      sources: [],
      finalManaCost: manaCost,
      projectedPool: { ...(player?.mana || {}) },
      remainingPool: canPay.ok ? spendMana(player?.mana || {}, manaCost, 0) : { ...(player?.mana || {}) },
    };
  } else {
    const planned = planManaPayment(player, manaCost, 0, {
      preserveColors: options.preserveColors || [],
      maxNodes: options.maxNodes || 10000,
    });
    payment = { ...planned, finalManaCost: manaCost };
  }
  ...
  return {
    ...
    reason: payment?.ok
      ? ''
      : `${state.players[defenderId]?.name || 'That player'} is protected by ${sourceNames.join(', ')}. Declaring ${count} attacker${count === 1 ? '' : 's'} requires ${manaCost || 'an attack payment'}, and the visible resources cannot pay it.`,
  };
}
...
function recognizedEffects(card) {
  ...
  if (attackTaxDescriptorsForPermanent(card).length) effects.push('Attack tax / combat restriction');

  const compiledStatics = compiledStaticAbilitiesForPermanent(card);
  if (compiledStatics.some((rule) => rule.type === 'CAST_PROHIBITION')) effects.push('Static casting restriction');
  if (compiledStatics.some((rule) => rule.type === 'BLOCK_PROHIBITION')) effects.push('Static blocking restriction');
  if (compiledStatics.some((rule) => rule.type === 'CAST_LIMIT')) effects.push('Spell-cast limit');
...
return { validateDeck, commanderCandidates, landEntryPlan, manaDevelopmentSnapshot, strategicPaymentColors, landPlayAllowance, landPlayLegality, effectiveSpellManaValue, compiledStaticAbilitiesForPermanent, compiledCastRestrictionViolations, compiledBlockRestrictionViolations, compiledBlockLegality, spellCastLegality, moveLegality, canPayMana, spendMana, planManaPayment, requirementToManaCost, buildCostPlan, planSpellPayment, applySpellPayment, attackTaxDescriptorsForPermanent, attackTaxSources, attackTaxPlan, attackLegality, attackGroupLegality, recognizedEffects, stackDestination, maximumHandSize };
})();
...
function ensureStateShape(next) {
  ...
  next.delayedEffects ||= [];
  next.monarchPlayerId = next.players?.[next.monarchPlayerId] ? next.monarchPlayerId : null;
...
const { applySpellPayment, attackGroupLegality, attackLegality, attackTaxPlan, compiledBlockLegality, landEntryPlan, landPlayLegality, manaDevelopmentSnapshot, planManaPayment, spellCastLegality } = __modules["./rules.js"];
...
function attackValue(state, playerId, card) {
  ...
  const attackTax = attackTaxPlan(state, playerId, opponentId, 1, {
    useSources: true,
    maxNodes: 3500,
  });
  if (attackTax.required && !attackTax.ok) return -999;

  const attackTaxPenalty = Number(attackTax.manaValue || 0) * 1.35;
  const blockers = opponent.zones.battlefield.filter((blocker) => compiledBlockLegality(state, blocker.controller || opponentId, blocker).legal && canBlock(card, blocker, opponent.zones.battlefield, player.zones.battlefield));
  if (!blockers.length) {
    const stats = effectiveStats(card, player.zones.battlefield);
    const traits = cardTraits(card);
    return stats.power * 1.25 + (traits.combatDamageTrigger ? 2.2 : 0) + (card.commander ? 0.65 : 0) - attackTaxPenalty;
  }
  return Math.max(...blockers.map((blocker) => combatTradeScore(card, [blocker], player.zones.battlefield, opponent.zones.battlefield)), -1.5) - attackTaxPenalty;
}
...
function generatedAttackPlans(state, playerId) {
  ...
  return uniqueActions(actions).filter((action) => {
    if (!attackGroupLegality(state, action.cardIds || []).legal) return false;
    const opponentId = otherPlayerId(state, playerId);
    const tax = attackTaxPlan(state, playerId, opponentId, (action.cardIds || []).length, {
      useSources: true,
      maxNodes: 5000,
    });
    return !tax.required || tax.ok;
  });
}
...
function chooseBlocks(state, attackerId, attackers) {
  ...
  for (const attacker of ordered) {
    ...
    const legal = available.filter((blocker) => compiledBlockLegality(state, blocker.controller || defender.id, blocker).legal && canBlock(attacker, blocker, defender.zones.battlefield, attackerPlayer.zones.battlefield));
    if (traits.menace) {
      let best = null;
...
function resolveCombat(state, playerId, cardIds) {
  ...
  for (const attacker of attackers) {
    ...
    if (state.monarchPlayerId === opponentId && Number(outcome.rawDamage || 0) > 0) state.monarchPlayerId = playerId;
...
function applyTacticalAction(original, playerId, action, options = {}) {
  ...
  if (action.type === 'attack') {
    ...
    const opponentId = otherPlayerId(state, playerId);
    const tax = attackTaxPlan(state, playerId, opponentId, legalIds.length, {
      useSources: true,
      maxNodes: 5000,
    });
    if (tax.required && !tax.ok) return { ok: false, state, reason: tax.reason || 'The attack tax cannot be paid.' };
    if (tax.required) applySpellPayment(state, playerId, tax.payment);
...
const { attackLegality, compiledBlockLegality, landEntryPlan, landPlayAllowance, landPlayLegality, manaDevelopmentSnapshot, planManaPayment, spendMana, spellCastLegality, strategicPaymentColors } = __modules["./rules.js"];
...
function visibleAttackValue(state, playerId, attacker) {
  const opponentId = otherPlayerId(state, playerId);
  const blockers = state.players[opponentId].zones.battlefield.filter((blocker) => compiledBlockLegality(state, blocker.controller || opponentId, blocker).legal && canBlock(attacker, blocker, state.players[opponentId].zones.battlefield, state.players[playerId].zones.battlefield));
...
function chooseBlockersForAttack(draft, playerId, attackers) {
  ...
  for (const attacker of orderedAttackers) {
    const legal = available.filter((blocker) => compiledBlockLegality(draft, blocker.controller || opponentId, blocker).legal && canBlock(attacker, blocker, blockBoard, attackBoard));
...
function defenseAdvice(state) {
  ...
  for (const attacker of ordered) {
    const legal = unused.filter((blocker) => compiledBlockLegality(state, blocker.controller || defender.id, blocker).legal && canBlock(attacker, blocker, defender.zones.battlefield, attackerPlayer.zones.battlefield));
...
const { applySpellPayment, attackLegality, attackTaxPlan, canPayMana, compiledBlockLegality, landEntryPlan, landPlayLegality, maximumHandSize, moveLegality, planManaPayment, spellCastLegality, spendMana, stackDestination } = __modules["./rules.js"];
const { activationMetadata, canActivateManaChoice, deepClone, formatManaBundle, isCreature, isLand, isPermanent, manaProductionChoices, shuffle, uid } = __modules["./utils.js"];
...
function playerControlsCommander(draft, playerId) {
  return (draft.players?.[playerId]?.zones?.battlefield || []).some((card) =>
    Boolean(card.commander) || (card.mutationPile || []).some((part) => Boolean(part.commander))
  );
}

function becomeMonarchInDraft(draft, playerId, reason = '') {
  if (!draft.players?.[playerId] || draft.players[playerId].lost) return false;
  const previous = draft.monarchPlayerId || null;
  draft.monarchPlayerId = playerId;
  if (previous !== playerId) {
    draft.log.unshift({
      id: uid('log'),
      time: new Date().toISOString(),
      text: `👑 ${draft.players[playerId].name} became the monarch${reason ? ` — ${reason}` : ''}.`,
    });
  }
  return true;
}

function fightEffectDescriptor(text = '') {
  const clean = String(effectResolutionText(text || '')).replace(/\s+/g, ' ').trim();

  const counterFight = clean.match(
    /^put (?:a|one|1) ([+-]\d+\/[+-]\d+) counter on target creature you control\.\s*then it fights target creature an opponent controls\.(.*)$/i
  );
  if (counterFight) {
    return {
      counter: counterFight[1],
      counterAmount: 1,
      ownTarget: 'creature-you-control',
      opponentTarget: 'creature-opponent-controls',
      delayedMonarch: /when the creature an opponent controls dies this turn,\s*if you control your commander,\s*you become the monarch/i.test(counterFight[2] || ''),
    };
  }

  const plainFight = clean.match(/^target creature you control fights target creature an opponent controls\.?(.*)$/i);
  if (plainFight) {
    return {
      counter: null,
      counterAmount: 0,
      ownTarget: 'creature-you-control',
      opponentTarget: 'creature-opponent-controls',
      delayedMonarch: /when the creature an opponent controls dies this turn,\s*if you control your commander,\s*you become the monarch/i.test(plainFight[1] || ''),
    };
  }
...
function fightTargetIsLegal(card, controllerId, scope) {
  if (!card || !isCreature(card)) return false;
  const traits = cardTraits(card);
  if (traits.shroud) return false;
  if (scope === 'opponent' && traits.hexproof) return false;
  const controller = card.controller || card.owner;
  if (scope === 'own' && controller !== controllerId) return false;
  if (scope === 'opponent' && controller === controllerId) return false;
  return true;
}

function moveDyingPermanentToOwnersGraveyard(draft, instanceId, actorId, reason = 'died') {
  const located = findCard(instanceId, draft);
  if (!located || located.zone !== 'battlefield') return null;

  const snapshot = deepClone(located.card);
  const moved = located.container.splice(located.index, 1)[0];

  clearCardRelations(draft, moved);
  moved.controller = moved.owner;
  moved.tapped = false;
  moved.attacking = false;
  moved.attackTargetPlayerId = null;
  moved.blocking = null;
  moved.blockedBy = [];
  ...
  recordZoneTransition(draft, {
    card: snapshot,
    actorId,
    subjectPlayerId: snapshot.owner,
    fromZone: 'battlefield',
    toZone: 'graveyard',
    castAttempt: false,
  });

  if (!moved.token) draft.players[moved.owner].zones.graveyard.push(moved);

  draft.log.unshift({
    id: uid('log'),
    time: new Date().toISOString(),
    text: `${snapshot.name} ${reason}.`,
  });

  return snapshot;
}

function resolveDelayedDeathEffectsInDraft(draft, leavingCard, destinationZone) {
  if (destinationZone !== 'graveyard' || !leavingCard?.instanceId) return;

  for (const effect of [...(draft.delayedEffects || [])]) {
    if (effect.type !== 'watch-card-dies-this-turn') continue;
    if (effect.targetCardId !== leavingCard.instanceId) continue;
    if (Number(effect.expiresTurn || 0) !== Number(draft.turnNumber || 0)) continue;

    if (effect.monarchIfCommander && playerControlsCommander(draft, effect.controllerId)) {
      becomeMonarchInDraft(draft, effect.controllerId, `${effect.sourceName || 'delayed effect'} resolved`);
    }

    draft.delayedEffects = (draft.delayedEffects || []).filter((item) => item.id !== effect.id);
  }
}

function fightDamageTraits(card, battlefield) {
  const traits = cardTraits(card, battlefield);
  return {
    power: Math.max(0, Number(traits.power || 0)),
    toughness: Number(traits.toughness || 0),
    deathtouch: Boolean(traits.deathtouch),
    lifelink: Boolean(traits.lifelink),
    infect: Boolean(traits.infect),
    wither: Boolean(traits.wither),
    indestructible: Boolean(traits.indestructible),
  };
}

function applyFightDamageToCreature(draft, source, target, sourceTraits) {
  const amount = Math.max(0, Number(sourceTraits.power || 0));
  if (!amount) return 0;

  if (sourceTraits.infect || sourceTraits.wither) {
    target.counters ||= {};
    target.counters['-1/-1'] = Number(target.counters['-1/-1'] || 0) + amount;
  } else {
    target.damageMarked = Number(target.damageMarked || 0) + amount;
    if (sourceTraits.deathtouch) target.deathtouchDamaged = true;
  }

  if (sourceTraits.lifelink) {
    const controllerId = source.controller || source.owner;
    if (draft.players?.[controllerId]) {
      draft.players[controllerId].life += amount;
      emitGameEvent(draft, { type: 'life-gain', actorId: controllerId, amount });
    }
  }
...
function creatureDiesFromMarkedDamage(card, battlefield) {
  const traits = fightDamageTraits(card, battlefield);
  if (traits.toughness <= 0) return true;
  if (traits.indestructible) return false;
  return Boolean(card.deathtouchDamaged) || Number(card.damageMarked || 0) >= traits.toughness;
}
...
function graveyardMoveDescriptor(text = '') {
  ...
  else if (/\b(?:to|into) your hand\b|\bto its owner'?s hand\b/i.test(clean)) destination = 'hand';
  else if (/\bto exile\b|\binto exile\b/i.test(clean)) destination = 'exile';
  else if (/\b(?:on|onto) top of (?:your|its owner'?s) library\b/i.test(clean)) destination = 'library-top';
  if (!destination) return null;
  return {
    filterText,
    maxCount,
...
function pendingEffectMoveAuthorization(state, card, fromZone, targetZone, targetPlayerId, libraryPosition = 'top') {
  const actualDestination = targetZone === 'library' && libraryPosition === 'top'
    ? 'library-top'
    : targetZone;
  ...
  for (const effect of state.pendingTriggers || []) {
    ...
    if (fromZone === 'library' && smart.kind === 'search-library') {
      if (Number(smart.maxCount || 1) !== 1) continue;
      if (smart.destination !== actualDestination) continue;
      if (!smart.cards?.some((item) => item.instanceId === card.instanceId)) continue;
      if (smart.playerId !== (card.owner || targetPlayerId)) continue;
      return { effect, kind: 'search-library', descriptor: smart };
    }

    if (fromZone === 'graveyard' && smart.kind === 'graveyard-move') {
      if (Number(smart.maxCount || 1) !== 1) continue;
      if (smart.destination !== actualDestination) continue;
      if (!smart.cards?.some((item) => item.instanceId === card.instanceId)) continue;
      if (targetZone === 'battlefield' && targetPlayerId !== effect.controllerId) continue;
      return { effect, kind: 'graveyard-move', descriptor: smart };
    }

    if (
      targetZone === 'graveyard'
      && smart.kind === 'sacrifice-permanent'
      && fromZone === 'battlefield'
      && Number(smart.amount || 1) === 1
      && smart.cards?.some((item) => item.instanceId === card.instanceId)
    ) {
      return { effect, kind: 'sacrifice-permanent', descriptor: smart };
    }

    if (
      targetZone === 'graveyard'
      && ['discard-card', 'hand-to-graveyard'].includes(smart.kind)
      && fromZone === 'hand'
      && Number(smart.amount || 1) === 1
      && smart.cards?.some((item) => item.instanceId === card.instanceId)
    ) {
      return { effect, kind: smart.kind, descriptor: smart };
    }
  }
...
function smartPendingEffectState(effect, state) {
  ...
  if (effect?.compiledProgram?.authoritative) {
    const compiledSearch = compiledProgramSearchState(effect, state);
    if (compiledSearch) return compiledSearch;

    const step = compiledCurrentStep(effect);
    if (step) {
      return {
        kind: 'compiled-unsupported-adapter',
        playerId: effect.controllerId,
        cards: [],
        step,
        noResultReason: `Compiler recognized ${step.type}, but the live adapter is not attached yet.`,
      };
    }
  }
  ...
  if (fight) {
    const ownCards = (state.players?.[effect.controllerId]?.zones?.battlefield || [])
      .filter((card) => fightTargetIsLegal(card, effect.controllerId, 'own'));
    const opponentCards = opponentPlayerIds(state, effect.controllerId)
      .flatMap((id) => state.players?.[id]?.zones?.battlefield || [])
      .filter((card) => fightTargetIsLegal(card, effect.controllerId, 'opponent'));
    return {
      kind: 'fight-creatures',
      playerId: effect.controllerId,
      ownCards,
      opponentCards,
      cards: [...ownCards, ...opponentCards],
      ...fight,
    };
  }
...
function resolveSmartLibraryEffect(effectId, cardIds = []) {
  ...
  updateState((draft) => {
    ...
    if (smart.shuffle) {
      // The chosen cards are already out of the library. This makes
      // Vampiric/Mystical Tutor wording resolve in the correct order:
      // remember the chosen card, shuffle the remainder, then place it on top.
      draft.players[smart.playerId].zones.library = shuffle(draft.players[smart.playerId].zones.library);
    }
    ...
    const draftEffect = draft.pendingTriggers.find((item) => item.id === effectId);
    if (draftEffect?.compiledProgram?.authoritative) {
      draftEffect.compiledIndex = Math.max(0, Number(draftEffect.compiledIndex || 0)) + 1;
      const progressed = advanceCompiledProgramInDraft(draft, draftEffect);
      checkLosses(draft);
      if (progressed.done) {
        draft.pendingTriggers = draft.pendingTriggers.filter((item) => item.id !== effectId);
      }
...
function resolveSmartFightEffect(effectId, ownTargetId, opponentTargetId) {
  const current = window.CommanderForge.getState();
  const effect = current.pendingTriggers.find((item) => item.id === effectId);
  if (!effect) return { ok: false, message: 'Pending fight effect not found.' };

  const descriptor = fightEffectDescriptor(effect.text || '');
  if (!descriptor) return { ok: false, message: 'This is not a supported fight effect.' };

  const currentOwn = findCard(ownTargetId, current);
  const currentOpponent = findCard(opponentTargetId, current);

  const ownLegal = Boolean(
    currentOwn
    && currentOwn.zone === 'battlefield'
    && fightTargetIsLegal(currentOwn.card, effect.controllerId, 'own')
  );

  const opponentLegal = Boolean(
    currentOpponent
    && currentOpponent.zone === 'battlefield'
    && fightTargetIsLegal(currentOpponent.card, effect.controllerId, 'opponent')
  );

  updateState((draft) => {
    draft.pendingTriggers = draft.pendingTriggers.filter((item) => item.id !== effectId);
    draft.delayedEffects ||= [];

    const ownLocated = ownLegal ? findCard(ownTargetId, draft) : null;
    const opponentLocated = opponentLegal ? findCard(opponentTargetId, draft) : null;

    if (!ownLocated && !opponentLocated) {
      draft.log.unshift({
        id: uid('log'),
        time: new Date().toISOString(),
        text: `${effect.sourceName}: both targets were illegal, so the effect did nothing.`,
      });
      return;
    }

    if (ownLocated && descriptor.counter) {
      ownLocated.card.counters ||= {};
      ownLocated.card.counters[descriptor.counter] =
        Number(ownLocated.card.counters[descriptor.counter] || 0)
        + Number(descriptor.counterAmount || 1);

      emitGameEvent(draft, {
        type: 'counter-added',
        actorId: effect.controllerId,
        card: ownLocated.card,
        counter: descriptor.counter,
        amount: Number(descriptor.counterAmount || 1),
      });
    }

    if (opponentLocated && descriptor.delayedMonarch) {
      draft.delayedEffects.push({
        id: uid('delayed'),
        type: 'watch-card-dies-this-turn',
        targetCardId: opponentTargetId,
        controllerId: effect.controllerId,
        monarchIfCommander: true,
        sourceName: effect.sourceName,
        expiresTurn: Number(draft.turnNumber || 1),
      });
    }

    if (ownLocated && opponentLocated) {
      const own = ownLocated.card;
      const opponent = opponentLocated.card;
      const ownBoard = draft.players[own.controller || own.owner]?.zones?.battlefield || [];
      const opponentBoard = draft.players[opponent.controller || opponent.owner]?.zones?.battlefield || [];

      // Fight damage is simultaneous, so both powers are captured first.
      const ownTraits = fightDamageTraits(own, ownBoard);
      const opponentTraits = fightDamageTraits(opponent, opponentBoard);

      applyFightDamageToCreature(draft, own, opponent, ownTraits);
      applyFightDamageToCreature(draft, opponent, own, opponentTraits);

      const ownDies = creatureDiesFromMarkedDamage(own, ownBoard);
      const opponentDies = creatureDiesFromMarkedDamage(opponent, opponentBoard);

      if (ownDies) moveDyingPermanentToOwnersGraveyard(draft, own.instanceId, effect.controllerId, 'died after fighting');
      if (opponentDies) moveDyingPermanentToOwnersGraveyard(draft, opponent.instanceId, effect.controllerId, 'died after fighting');
    ...
    draft.log.unshift({
      id: uid('log'),
      time: new Date().toISOString(),
      text: `${effect.sourceName}: ${ownLocated ? ownLocated.card.name : 'first target'}${descriptor.counter ? ` got a ${descriptor.counter} counter` : ''}${ownLocated && opponentLocated ? ` and fought ${opponentLocated.card.name}` : '; the creatures did not fight because only one target remained legal'}.`,
    });
  }, { log: `${effect.sourceName}: resolved its targeted fight instructions automatically.` });

  return {
    ok: true,
    message: ownLegal && opponentLegal
      ? 'Fight effect resolved automatically.'
      : 'Only one target remained legal, so the creatures did not fight.',
  };
}
...
function resolveSmartPendingEffectMulti(effectId, cardIds = [], powerOverrides = {}) {
  ...
  if (['sacrifice-permanent', 'discard-card', 'hand-to-graveyard'].includes(smart.kind)) {
    ...
    updateState((draft) => {
      const sacrificeBatchSerial = smart.kind === 'sacrifice-permanent' ? uid('sacrifice-batch') : null;

      for (const id of ids) {
        const located = findCard(id, draft);
        if (!located) continue;

        if (smart.kind === 'sacrifice-permanent' && located.zone === 'battlefield') {
          sacrificePermanentInDraft(draft, id, smart.victimId || effect.controllerId, {
            eventSerial: sacrificeBatchSerial,
            reason: 'was sacrificed',
          });
          continue;
        }
        ...
        if (smart.kind === 'discard-card') {
          emitGameEvent(draft, { type: 'discard', actorId: smart.victimId, card });
        }
      }
...
function makePendingEffect(draft, {
  ...
  const compiler = globalThis.CommanderForgeOracleCompilerV7;
  let program = compiledProgram ? deepClone(compiledProgram) : null;

  if (!program && compiler?.compileProgram) {
    try {
      const candidate = compiler.compileProgram(effectResolutionText(text), {
        cardName: sourceCard?.name || '',
      });
      if (candidate?.authoritative && candidate?.keepTogether) program = candidate;
    } catch (error) {
      console.warn('[Commander Forge Oracle Compiler] pending-effect compile failed', error);
    }
  }
  ...
  return {
    ...
    compiledIndex: Math.max(0, Number(compiledIndex || 0)),
...
function compiledCurrentStep(effect) {
  const steps = effect?.compiledProgram?.steps || [];
  return steps[Math.max(0, Number(effect?.compiledIndex || 0))] || null;
}

function compiledProgramController(effect, draft) {
  return effect?.controllerId || draft.activePlayerId;
}

function compiledProgramAutomaticStep(draft, effect, step) {
  const playerId = compiledProgramController(effect, draft);
  const player = draft.players?.[playerId];
  if (!player || !step) return false;

  const amount = Math.max(0, Number(step.amount || 0));

  if (step.type === 'LOSE_LIFE') {
    player.life -= amount;
    emitGameEvent(draft, { type: 'life-loss', actorId: playerId, amount });
    draft.log.unshift({
      id: uid('log'),
      time: new Date().toISOString(),
      text: `${effect.sourceName}: ${player.name} lost ${amount} life.`,
    });
    return true;
  }

  if (step.type === 'GAIN_LIFE') {
    player.life += amount;
    emitGameEvent(draft, { type: 'life-gain', actorId: playerId, amount });
    draft.log.unshift({
      id: uid('log'),
      time: new Date().toISOString(),
      text: `${effect.sourceName}: ${player.name} gained ${amount} life.`,
    });
    return true;
  }
  ...
  if (step.type === 'DRAW') {
    ...
    draft.log.unshift({
      id: uid('log'),
      time: new Date().toISOString(),
      text: `${effect.sourceName}: ${player.name} drew ${amount} card${amount === 1 ? '' : 's'}.`,
    });
    return true;
  }
  ...
  if (step.type === 'MILL') {
    ...
    draft.log.unshift({
      id: uid('log'),
      time: new Date().toISOString(),
      text: `${effect.sourceName}: ${player.name} milled ${amount} card${amount === 1 ? '' : 's'}.`,
    });
    return true;
  }

  if (step.type === 'SHUFFLE_LIBRARY') {
    player.zones.library = shuffle(player.zones.library);
    recordPublicEvent(draft, {
      type: 'shuffled',
      actorId: playerId,
      subjectPlayerId: playerId,
    ...
    draft.log.unshift({
      id: uid('log'),
      time: new Date().toISOString(),
      text: `${effect.sourceName}: ${player.name} shuffled their library.`,
    });
    return true;
  }

  if (step.type === 'BECOME_MONARCH') {
    becomeMonarchInDraft(draft, playerId, effect.sourceName);
    return true;
  }
...
function advanceCompiledProgramInDraft(draft, effect) {
  if (!effect?.compiledProgram?.steps?.length) return { done: false, advanced: 0 };

  let advanced = 0;

  while (effect.compiledIndex < effect.compiledProgram.steps.length) {
    const step = compiledCurrentStep(effect);
    if (!step) break;

    if (step.type === 'SEARCH_LIBRARY') break;
    if (!compiledProgramAutomaticStep(draft, effect, step)) break;

    effect.compiledIndex += 1;
    advanced += 1;
  }

  return {
    done: effect.compiledIndex >= effect.compiledProgram.steps.length,
    advanced,
  };
}

function compiledProgramSearchState(effect, state) {
  const step = compiledCurrentStep(effect);
  if (!step || step.type !== 'SEARCH_LIBRARY') return null;

  const playerId = effect.controllerId;
  const cards = (state.players?.[playerId]?.zones?.library || [])
    .filter((card) => libraryCardMatchesSearch(card, {
      filterText: step.filterText || 'card',
    }));

  return {
    kind: 'search-library',
    compiled: true,
    compiledStep: deepClone(step),
    playerId,
    cards,
    filterText: step.filterText || 'card',
    maxCount: Math.max(1, Number(step.maxCount || 1)),
    zeroAllowed: Boolean(step.zeroAllowed),
    destination: step.destination,
    tapped: Boolean(step.tapped),
    reveal: Boolean(step.reveal),
    shuffle: Boolean(step.shuffle),
    shuffleBeforePlacement: Boolean(step.shuffleBeforePlacement),
  };
}
...
function triggerPerspectiveMatches(line, sourceCard, event) {
  ...
  const youDidIt = /\byou\s+(?:cast|draw|gain|lose|attack|activate|mill|discard|sacrifice)/.test(text)
    || /\bwhenever you\s+(?:cast|draw|gain|lose|attack|activate|mill|discard|sacrifice)/.test(text);
  if (youDidIt && controller !== actor) return false;

  const opponentDidIt = /\b(?:an|each|target)?\s*opponent\s+(?:casts?|draws?|gains?|loses?|attacks?|activates?|mills?|discards?|sacrifices?)/.test(text);
...
function lineMatchesGameEvent(line, sourceCard, event) {
  ...
  if (event.type === 'sacrifice') {
    if (!/\bsacrific(?:e|es|ed)\b/.test(text)) return false;
    if (/\b(?:another|other)\b/.test(text) && sourceCard.instanceId === event.card?.instanceId) return false;
    if (!cardMatchesEventSubject(event.card, condition)) return false;
    return true;
  }
...
function emitGameEvent(draft, event) {
  ...
  if (['cast', 'sacrifice'].includes(event.type) && event.card && !sources.some((card) => card.instanceId === event.card.instanceId)) sources.push(event.card);
  const seen = new Set();
...
function enqueueBattlefieldLeaveEffects(draft, leavingCard, destinationZone = 'removed') {
  ...
  resolveDelayedDeathEffectsInDraft(draft, leavingCard, destinationZone);
}
...
function sacrificePermanentInDraft(draft, instanceId, actorId, {
  eventSerial = null,
  reason = 'was sacrificed',
} = {}) {
  const located = findCard(instanceId, draft);
  ...
  const snapshot = deepClone(located.card);
  const sacrificed = located.container.splice(located.index, 1)[0];
  ...
  sacrificed.attackTargetPlayerId = null;
  sacrificed.blocking = null;
  sacrificed.blockedBy = [];

  // A sacrifice to the graveyard is still a battlefield-leave event and, for
  // creatures, normally a "dies" event. Tokens reach the graveyard
  // momentarily for trigger purposes and then cease to exist.
  enqueueBattlefieldLeaveEffects(draft, snapshot, 'graveyard');
  ...
  emitGameEvent(draft, {
    ...
    actorId,
    card: snapshot,
    _forgeEventSerial: eventSerial || undefined,
  });
  ...
  draft.log.unshift({
    id: uid('log'),
    time: new Date().toISOString(),
    text: sacrificed.token
      ? `${snapshot.name} ${reason}, then ceased to exist.`
      : `${snapshot.name} ${reason}.`,
  });

  return snapshot;
}

function sacrificePermanent(instanceId) {
  const current = window.CommanderForge.getState();
  const found = findCard(instanceId, current);

  if (!found || found.zone !== 'battlefield') {
    return { ok: false, message: 'Only a battlefield permanent can be sacrificed.' };
  }

  const actorId = found.card.controller || found.playerId;
  const authorization = pendingEffectMoveAuthorization(
    current,
    found.card,
    'battlefield',
    'graveyard',
  ...
  updateState((draft) => {
    const sacrificed = sacrificePermanentInDraft(draft, instanceId, actorId, {
      reason: 'was sacrificed',
    });
    if (!sacrificed) return;

    // If a currently pending "sacrifice a permanent/creature/etc." instruction
    // required exactly this card, the explicit Sacrifice button completes it.
    if (authorization?.kind === 'sacrifice-permanent') {
      consumePendingMoveEffect(draft, authorization, sacrificed);
    }

    revalidateAuraAttachments(draft);
    applyAuraControlEffects(draft);

    if (draft.selected?.instanceId === instanceId) {
      draft.selected = sacrificed.token ? null : { instanceId };
    }

    checkLosses(draft);
  }, { log: `${found.card.name}: sacrificed by ${current.players[actorId]?.name || actorId}.` });

  return {
    ok: true,
    message: found.card.token
      ? `${found.card.name} was sacrificed and ceased to exist.`
      : `${found.card.name} was sacrificed to its owner's graveyard.`,
  };
}
...
function moveCard(instanceId, targetPlayerId, targetZone, { force = false, libraryPosition = 'top', enterTapped = false, countsAsLandPlay = true, xValue = null, auraTargetId = null, skipPayment = false, testOverride = false } = {}) {
  ...
  // If this movement is exactly what a pending card effect is asking for,
  // resolve through the effect engine rather than the generic zone mover.
  // This is critical for ordered tutors such as Vampiric Tutor: the resolver
  // remembers the chosen card, shuffles the remainder, puts it on top, and
  // then continues mandatory follow-up instructions such as losing 2 life.
  if (!force && earlyPendingMoveAuthorization?.kind === 'search-library' && source.zone === 'library') {
    return resolveSmartLibraryEffect(earlyPendingMoveAuthorization.effect.id, [instanceId]);
  }

  if (!force && earlyPendingMoveAuthorization?.kind === 'graveyard-move' && source.zone === 'graveyard') {
    return resolveSmartPendingEffect(earlyPendingMoveAuthorization.effect.id, instanceId);
  }
...
function toggleAttack(instanceId, targetPlayerId = null) {
  ...
  let attackTax = null;
  if (declaring && targetPlayerId) {
    attackTax = attackTaxPlan(
      current,
      found.card.controller,
      targetPlayerId,
      1,
      {
        useSources: current.settings.manaMode !== 'manual',
        maxNodes: 8000,
      }
    );

    if (attackTax.required && !attackTax.ok) {
      const manualHint = current.settings.manaMode === 'manual'
        ? ` Add ${attackTax.manaCost || 'the required mana'} to the floating pool before declaring this attacker.`
        : '';
      return {
        ok: false,
        message: `${attackTax.reason || 'This attack has an additional mana payment.'}${manualHint}`,
        attackTax,
      };
    }
  }
  ...
  updateState((draft) => {
    ...
    if (declaring && attackTax?.required) {
      applySpellPayment(draft, located.card.controller, attackTax.payment);
      draft.log.unshift({
        id: uid('log'),
        time: new Date().toISOString(),
        text: `${draft.players[located.card.controller]?.name || located.card.controller} paid ${attackTax.manaCost} to attack ${draft.players[targetPlayerId]?.name || targetPlayerId} through ${attackTax.sourceNames.join(', ')}.`,
      });
    }
...
Test Override is enabled. Advance the phase anyway?`);
  ...
  updateState((draft) => {
    if (nextIndex === 0) {
      ...
      draft.turnNumber += 1;
      draft.delayedEffects = (draft.delayedEffects || []).filter(
        (effect) => Number(effect.expiresTurn || 0) >= Number(draft.turnNumber || 0)
      );
    ...
    if (PHASES[nextIndex].id === 'end' && draft.monarchPlayerId === active.id) {
      drawCards(draft, active.id, 1);
      emitGameEvent(draft, { type: 'draw', actorId: active.id, amount: 1, reason: 'monarch' });
      draft.log.unshift({
        id: uid('log'),
        time: new Date().toISOString(),
        text: `👑 ${active.name} drew a card for being the monarch.`,
      });
    }
...
function resolveStackTop() {
  ...
  updateState((draft) => {
    ...
    if (destination === 'battlefield' && isAura(resolved)) {
      ...
      // Oracle Compiler V7 keeps safe ordered programs together, rather than
      // splitting choices and their mandatory follow-ups into unrelated prompts.
      let compiledQueued = false;
      const compiler = globalThis.CommanderForgeOracleCompilerV7;

      if (compiler?.compileProgram) {
        try {
          const program = compiler.compileProgram(resolved.oracleText || '', {
            cardName: resolved.name,
          });

          if (program?.authoritative && program?.keepTogether) {
            queuePendingEffect(draft, {
              sourceCard: resolved,
              controllerId: resolved.controller,
              kind: 'compiled-spell-effect',
              text: resolved.oracleText || '',
              compiledProgram: program,
            });
            compiledQueued = true;
          }
        } catch (error) {
          console.warn('[Commander Forge Oracle Compiler] spell compile failed', resolved.name, error);
        }
      }

      if (!compiledQueued) {
        const executableLines = oracleAbilityLines(resolved)
          .filter((line) => !/^(?:when|whenever|at)\b/i.test(line) && !line.includes(':'));
...
function activateBattlefieldAbility(instanceId, abilityIndex = 0, costChoices = {}) {
  ...
  if (
    metadata.requiresTap
    && isCreature(found.card)
    && found.card.summoningSick
    && !cardHasHaste(found.card)
  ) {
    return { ok: false, message: `${found.card.name} has summoning sickness and cannot pay a {T} activation cost yet.` };
  }
  ...
  updateState((draft) => {
    ...
    const sacrificePermanentAsCost = (cardId) => sacrificePermanentInDraft(
      draft,
      cardId,
      playerId,
      { reason: 'was sacrificed as an activation cost' }
    );
...
function assignBlocker(blockerId, attackerId) {
  ...
  const removingExistingBlock = blocker.card.blocking === attackerId;

  if (!removingExistingBlock) {
    const blockerPlayerId = blocker.card.controller || blocker.playerId;
    const staticBlock = compiledBlockLegality(current, blockerPlayerId, blocker.card);
    if (!staticBlock.legal) return { ok: false, message: staticBlock.reasons.join(' ') };

    const blockerBoard = current.players[blockerPlayerId]?.zones?.battlefield || [];
    const attackerPlayerId = attacker.card.controller || attacker.playerId;
    const attackerBoard = current.players[attackerPlayerId]?.zones?.battlefield || [];

    if (!canBlock(attacker.card, blocker.card, blockerBoard, attackerBoard)) {
      return { ok: false, message: `${blocker.card.name} cannot legally block ${attacker.card.name}.` };
    }
  }
...
// ---- main.js ----
globalThis.CommanderForgeBuildVersion = '6.13.0';
(() => {
...
const { attackTaxPlan, commanderCandidates, landPlayAllowance, maximumHandSize, moveLegality, recognizedEffects, validateDeck } = __modules["./rules.js"];
...
const MULTIPLAYER_APP_VERSION = '6.13.0-mp7';
...
const multiplayer = {
  ...
  seatReservations: Object.create(null),
  reservedPlayerIds: [],
  reconnectClaim: null,
  playerCount: 4,
...
const MULTIPLAYER_RECONNECT_STORAGE_KEY = 'commander-forge-multiplayer-reconnect-v1';
const MULTIPLAYER_SEAT_RESERVATION_MS = 15 * 60 * 1000;

function generateReconnectToken() {
  const bytes = new Uint8Array(24);
  globalThis.crypto?.getRandomValues?.(bytes);
  return Array.from(bytes, (value, index) => Number(value || (Math.random() * 256 + index) % 256).toString(16).padStart(2, '0')).join('');
}

function readReconnectStore() {
  try {
    const parsed = JSON.parse(localStorage.getItem(MULTIPLAYER_RECONNECT_STORAGE_KEY) || '{}');
...
function writeReconnectStore(store) {
  try { localStorage.setItem(MULTIPLAYER_RECONNECT_STORAGE_KEY, JSON.stringify(store || {})); } catch {}
}

function reconnectClaimForRoom(codeValue) {
  const code = normalizeInviteCode(codeValue);
  if (!code) return null;
  const claim = readReconnectStore()[code];
  if (!claim?.playerId || !claim?.token) return null;
  return { roomCode: code, playerId: claim.playerId, token: claim.token, savedAt: Number(claim.savedAt || 0), gameId: claim.gameId || '' };
}

function rememberReconnectClaim(roomCodeValue, playerId, token, gameId = '') {
  const roomCode = normalizeInviteCode(roomCodeValue);
  if (!roomCode || !playerId || !token) return;
  const store = readReconnectStore();
  store[roomCode] = { playerId, token, gameId: gameId || store[roomCode]?.gameId || '', savedAt: Date.now() };
  const entries = Object.entries(store).sort((a, b) => Number(b[1]?.savedAt || 0) - Number(a[1]?.savedAt || 0));
  writeReconnectStore(Object.fromEntries(entries.slice(0, 8)));
  multiplayer.reconnectClaim = { roomCode, playerId, token, gameId: store[roomCode].gameId || '' };
}

function currentReconnectMetadata() {
  const claim = multiplayer.reconnectClaim || reconnectClaimForRoom(multiplayer.roomCode);
  if (!claim || normalizeInviteCode(claim.roomCode) !== normalizeInviteCode(multiplayer.roomCode)) return {};
  return { reconnectPlayerId: claim.playerId, reconnectToken: claim.token };
}

function seatReservationActive(playerId) {
  const reservation = multiplayer.seatReservations?.[playerId];
  if (!reservation?.token) return false;
  if (multiplayer.connections?.[playerId]?.open) return true;
  if (getState()?.onlineGameId) return true;
  const disconnectedAt = Number(reservation.disconnectedAt || 0);
  return !disconnectedAt || Date.now() - disconnectedAt < MULTIPLAYER_SEAT_RESERVATION_MS;
}

function releaseExpiredSeatReservation(playerId) {
  if (seatReservationActive(playerId)) return false;
  delete multiplayer.seatReservations[playerId];
  delete multiplayer.remoteDrafts[playerId];
  if (playerId === multiplayerRemotePlayerId()) multiplayer.remoteDraft = null;
  delete multiplayer.readyByPlayer[playerId];
  return true;
}

function reservationMatches(playerId, token) {
  if (!playerId || !token) return false;
  const reservation = multiplayer.seatReservations?.[playerId];
  return Boolean(reservation?.token && reservation.token === token);
}

function releaseReservedSeat(playerId) {
  if (multiplayer.role !== 'host' || !multiplayerSeatIds().slice(1).includes(playerId)) return { ok: false, message: 'Only the host can free a guest seat.' };
  if (getState()?.onlineGameId) return { ok: false, message: 'Seats stay locked once the game has started.' };
  if (multiplayer.connections?.[playerId]?.open) return { ok: false, message: 'That player is still connected.' };
  delete multiplayer.seatReservations[playerId];
  delete multiplayer.remoteDrafts[playerId];
  if (playerId === multiplayerRemotePlayerId()) multiplayer.remoteDraft = null;
  delete multiplayer.readyByPlayer[playerId];
  multiplayer.reservedPlayerIds = (multiplayer.reservedPlayerIds || []).filter((id) => id !== playerId);
  ui.drafts[playerId] = createDraft(`Player ${String(playerId).replace(/^p/, '')}`);
  broadcastLobbyState();
  render();
  return { ok: true, message: `Player ${String(playerId).replace(/^p/, '')}'s seat is open again.` };
}
...
function multiplayerLobbySnapshot() {
  ...
  const reservedPlayerIds = multiplayerSeatIds().slice(1).filter((id) => {
    releaseExpiredSeatReservation(id);
    return seatReservationActive(id) && !connectedPlayerIds.includes(id);
  });
  multiplayer.reservedPlayerIds = reservedPlayerIds;
  return {
...
function applyAuthoritativeNetworkState(incoming, revision = multiplayer.revision, metadata = {}) {
  ...
  try {
    ...
    if (multiplayer.role === 'guest' && multiplayer.reconnectClaim?.token && getState()?.onlineGameId) {
      rememberReconnectClaim(multiplayer.roomCode, multiplayer.localPlayerId, multiplayer.reconnectClaim.token, getState().onlineGameId);
    }
  } finally {
    multiplayer.applyingRemote = false;
  }
...
function resetMultiplayerLobbyState() {
  ...
  multiplayer.seatReservations = Object.create(null);
  multiplayer.reservedPlayerIds = [];
  multiplayer.reconnectClaim = null;
...
function nextAvailableGuestSeat() {
  for (const id of multiplayerSeatIds().slice(1)) {
    releaseExpiredSeatReservation(id);
    if (!multiplayer.connections?.[id] && !seatReservationActive(id)) return id;
  }
  return null;
}

function sendSeatAssignment(playerId, { resumed = false } = {}) {
  ...
  try {
    connection.send({
      ...
      reconnectToken: multiplayer.seatReservations?.[playerId]?.token || '',
      resumed: Boolean(resumed),
      gameId: getState()?.onlineGameId || '',
...
function attachHostGuestConnection(connection) {
  ...
  const requestedPlayerId = multiplayerSeatIds().includes(metadata.reconnectPlayerId) ? metadata.reconnectPlayerId : null;
  const reconnectToken = String(metadata.reconnectToken || '');
  const validReconnect = Boolean(requestedPlayerId && reconnectToken && reservationMatches(requestedPlayerId, reconnectToken) && seatReservationActive(requestedPlayerId));
  let playerId = validReconnect ? requestedPlayerId : nextAvailableGuestSeat();
  let resumed = validReconnect;

  if (playerId && multiplayer.connections?.[playerId]?.open) {
    const rejectSeatInUse = () => {
      try { connection.send({ version: MULTIPLAYER_APP_VERSION, type: 'seat-in-use', playerId }); } catch {}
      setTimeout(() => { try { connection.close(); } catch {} }, 100);
    };
    connection.on('open', rejectSeatInUse);
    if (connection.open) queueMicrotask(rejectSeatInUse);
    return;
  }
  ...
  if (!resumed) {
    multiplayer.seatReservations[playerId] = {
    ...
    multiplayer.seatReservations[playerId].disconnectedAt = 0;
  }
  ...
  const onHostConnectionOpen = () => {
    ...
    sendSeatAssignment(playerId, { resumed });
    broadcastLobbyState();
    if (getState()?.onlineGameId) {
      sendNetwork({ type: 'state', revision: multiplayer.revision, state: stateForNetwork() }, playerId);
      broadcastTurnSync(playerId);
    }
    render();
    toast(resumed
      ? `Player ${String(playerId).replace(/^p/, '')} reconnected.`
      : `Player ${String(playerId).replace(/^p/, '')} connected.`);
  };
  ...
  connection.on('close', () => {
    const wasCurrentConnection = multiplayer.connections[playerId] === connection;
    if (!wasCurrentConnection) return;
    delete multiplayer.connections[playerId];
    const reservation = multiplayer.seatReservations?.[playerId];
    if (reservation) reservation.disconnectedAt = Date.now();
    ...
    broadcastLobbyState();
    render();
    toast(`Player ${String(playerId).replace(/^p/, '')} disconnected. Their seat is reserved for reconnect.`, true);
  });
...
function scheduleGuestHostReconnect(reason = '') {
  ...
  if (!multiplayer.peer?.open) {
    ...
    multiplayer.lastError = reason || 'Reconnecting to the multiplayer signaling service…';
    render();
...
function attachGuestConnection(connection) {
  ...
  const onGuestConnectionOpen = () => {
    ...
    if (!multiplayer.localPlayerId) requestGuestSeatAssignment({ immediate: true });
    else {
      // A transient reconnect already knows its local player ID. Ask once anyway
      // so a lost seat-assigned packet cannot leave the guest half-reconnected.
      try {
        connection.send({
          version: MULTIPLAYER_APP_VERSION,
          type: 'seat-request',
  ...
  connection.on('close', () => {
    if (multiplayer.connection !== connection) return;
    multiplayer.connection = null;
    if (multiplayer.seatAssignmentTimer) clearTimeout(multiplayer.seatAssignmentTimer);
    multiplayer.seatAssignmentTimer = null;

    if (!opened && multiplayer.role === 'guest') {
    ...
    multiplayer.status = 'disconnected';
    render();
    toast('Multiplayer connection closed. Reconnecting automatically…', true);
    scheduleGuestHostReconnect('Connection lost. Reconnecting to your reserved seat…');
  });

  connection.on('error', (error) => {
    multiplayer.guestConnecting = false;
    if (guestHostPeerUnavailable(error)) {
...
function createGuestPeerAndConnect() {
  ...
  multiplayer.peer.on('error', safeNetworkCallback('Guest PeerJS error', (error) => {
    if (multiplayer.role === 'guest' && guestHostPeerUnavailable(error)) {
      multiplayer.guestConnecting = false;
...
function joinOnlineHost(codeValue) {
  ...
  multiplayer.reconnectClaim = reconnectClaimForRoom(code);
  // The stored claim suggests a seat, but the host must confirm it before this browser controls that player.
  multiplayer.localPlayerId = null;
...
function applyLobbySnapshot(lobby = {}) {
  ...
  multiplayer.reservedPlayerIds = [...(lobby.reservedPlayerIds || [])];
...
function handleNetworkMessage(message, senderPlayerId = null) {
  ...
  if (message.type === 'seat-in-use') {
    multiplayer.status = 'error';
    multiplayer.lastError = `Player ${String(message.playerId || '').replace(/^p/, '') || '?'} is already connected on another browser.`;
    render();
    return;
  }
  if (message.type === 'seat-assigned' && multiplayer.role === 'guest') {
    ...
    multiplayer.status = 'connected';
    if (message.reconnectToken) rememberReconnectClaim(multiplayer.roomCode, message.playerId, message.reconnectToken, message.gameId || getState()?.onlineGameId || '');
    ensureMultiplayerSeatUi();
    ...
    } else if (getState()?.onlineGameId) {
      sendNetwork({ type: 'request-state' });
    }
    render();
    toast(`${message.resumed ? 'Reconnected' : 'Joined'} as Player ${String(multiplayer.localPlayerId).replace(/^p/, '')}.`);
    return;
  }
  if ((message.type === 'hello' || message.type === 'seat-request') && multiplayer.role === 'host') {
    const playerId = senderPlayerId;
    if (playerId && multiplayer.connections?.[playerId]?.open) {
      sendSeatAssignment(playerId, { resumed: Boolean(message.reconnectToken && reservationMatches(playerId, message.reconnectToken)) });
    }
    broadcastLobbyState();
    return;
  }
...
function renderOnlineRemoteDeckStatus(playerId) {
  ...
  const reserved = !connected && ((multiplayer.reservedPlayerIds || []).includes(playerId) || (multiplayer.role === 'host' && seatReservationActive(playerId)));
  ...
  const stateClass = reserved ? 'deck-ready' : ready ? 'ready' : deckValid ? 'deck-ready' : connected ? 'connected' : 'empty';
  const connectionText = connected ? 'Connected' : reserved ? 'Disconnected · seat reserved' : 'Waiting for player…';
  const pillText = reserved ? '↻ RECONNECT' : ready ? '✓ READY' : deckValid ? 'Deck loaded' : connected ? 'Setting up' : 'Empty seat';
  return `<section class="multiplayer-seat-card ${stateClass}">
    <div class="multiplayer-seat-head"><span class="seat-number">P${seatNumber}</span><div><strong>${escapeHtml(draft?.name || `Player ${seatNumber}`)}</strong><span>${connectionText}</span></div><span class="seat-ready-pill ${ready && !reserved ? 'ready' : ''}">${pillText}</span></div>
    ...
    ${reserved && multiplayer.role === 'host' ? `<button class="btn small-btn ghost" data-action="release-reserved-seat" data-player-id="${playerId}">Free seat</button>` : ''}
  </section>`;
}
...
function botCreatureStat(card, key) {
  const base = Number(card?.[key] || 0);
  const counters = card?.counters || {};
  let delta = 0;
  ...
  for (const [name, raw] of Object.entries(counters)) {
    ...
    if (!match) continue;
    delta += Number(match[key === 'power' ? 1 : 2] || 0) * Number(raw || 0);
  }

  return base + delta;
}

function botTargetableCreature(card, controllerId, scope) {
  if (!card || !isCreature(card)) return false;

  const keywordText = (card.manualKeywords || []).join(' ');
  const text = `${card.oracleText || ''}\n${keywordText}`.toLocaleLowerCase();

  if (/\bshroud\b/.test(text)) return false;

  const controller = card.controller || card.owner;
  if (scope === 'own' && controller !== controllerId) return false;
  if (scope === 'opponent' && controller === controllerId) return false;
  if (scope === 'opponent' && /\bhexproof\b/.test(text)) return false;

  return true;
}

function botFightTargetPairScore(own, opponent) {
  const ownPower = Math.max(0, botCreatureStat(own, 'power') + 1);
  const ownToughness = botCreatureStat(own, 'toughness') + 1;
  const oppPower = Math.max(0, botCreatureStat(opponent, 'power'));
  const oppToughness = botCreatureStat(opponent, 'toughness');

  const ownText = String(own.oracleText || '').toLocaleLowerCase();
  const oppText = String(opponent.oracleText || '').toLocaleLowerCase();

  const killsOpponent = ownPower >= oppToughness || (/\bdeathtouch\b/.test(ownText) && ownPower > 0);
  const ownSurvives = /\bindestructible\b/.test(ownText) || oppPower < ownToughness || oppPower === 0;
  const opponentIndestructible = /\bindestructible\b/.test(oppText);

  const opponentValue =
    botCardRoleScore(opponent)
    + Number(opponent.manaValue || 0) * 0.45
    + Math.max(0, oppPower + oppToughness) * 0.25
    + (opponent.commander ? 2.2 : 0);

  const ownValue =
    botCardRoleScore(own)
    + Number(own.manaValue || 0) * 0.3
    + Math.max(0, ownPower + ownToughness) * 0.18
    + (own.commander ? 1.5 : 0);

  let score = 1.2 + opponentValue;
  if (killsOpponent && !opponentIndestructible) score += 8;
  else score -= 3.5;

  if (ownSurvives) score += 5;
  else score -= ownValue * 1.35;

  return score;
}

function botFightSpellTargetPlan(card, state, controllerId) {
  const text = String(card?.oracleText || '').replace(/\s+/g, ' ').trim();

  const isFight =
    /put (?:a|one|1) [+-]\d+\/[+-]\d+ counter on target creature you control\.\s*then it fights target creature an opponent controls/i.test(text)
    || /target creature you control fights target creature an opponent controls/i.test(text);

  if (!isFight) return null;

  const ownCards = (state.players?.[controllerId]?.zones?.battlefield || [])
    .filter((candidate) => botTargetableCreature(candidate, controllerId, 'own'));

  const opponentCards = Object.keys(state.players || {})
    .filter((id) => id !== controllerId && !state.players[id]?.lost)
    .flatMap((id) => state.players[id]?.zones?.battlefield || [])
    .filter((candidate) => botTargetableCreature(candidate, controllerId, 'opponent'));

  if (!ownCards.length || !opponentCards.length) {
    return {
      kind: 'fight',
      ok: false,
  ...
  for (const own of ownCards) {
    for (const opponent of opponentCards) {
      const score = botFightTargetPairScore(own, opponent);
      if (!best || score > best.score) best = { own, opponent, score };
    }
  }
  ...
  return {
    ...
    ok: Boolean(best),
    requiresTargets: true,
    ownTargetId: best?.own?.instanceId || null,
    opponentTargetId: best?.opponent?.instanceId || null,
    targetIds: best ? [best.own.instanceId, best.opponent.instanceId] : [],
    targetNames: best ? [best.own.name, best.opponent.name] : [],
  };
}

function botSpellTargetPlan(card, state, controllerId) {
  return botFightSpellTargetPlan(card, state, controllerId);
}
...
function botCastWithPriority(step, scoredResult = step) {
  ...
  const targetPlan = botSpellTargetPlan(found.card, state, bot.playerId);
  if (targetPlan?.requiresTargets && !targetPlan.ok) {
    bot.lastAction = `Held ${found.card.name}`;
    bot.lastReason = 'The spell currently has no legal target pair.';
    return false;
  }
  ...
  bot.pendingSpell = {
    cardId: step.cardId,
    action: deepClone(step),
    label: step.label || `Cast ${found.card.name}`,
    targetPlan: targetPlan ? deepClone(targetPlan) : null,
  };
...
function botActivatedAbilityCostChoices(card, abilityIndex, state) {
  const abilities = battlefieldActivatedAbilities(card.instanceId);
  const ability = abilities[Number(abilityIndex || 0)] || '';
  const colon = ability.indexOf(':');
  const costText = colon >= 0 ? ability.slice(0, colon).trim() : '';

  const choices = {};

  const selfName = String(card.name || '').replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  const sacrificesSelf =
    new RegExp(`\\bsacrifice\\s+(?:${selfName}|this (?:creature|artifact|permanent|land|enchantment))\\b`, 'i').test(costText)
    || /\bsacrifice\s+(?:this artifact|this permanent|this land|this creature|this enchantment)\b/i.test(costText);

  const sacrificeMatch = costText.match(/\bsacrifice\s+(?:a|an|one)\s+(creature|artifact|permanent|land|enchantment)\b/i);
  const sacrificeType = sacrificeMatch?.[1]?.toLocaleLowerCase() || null;

  if (sacrificeType && !sacrificesSelf) {
    const battlefield = state.players?.[bot.playerId]?.zones?.battlefield || [];
    const candidates = battlefield.filter((candidate) => {
      if (candidate.instanceId === card.instanceId) return false;
      const type = String(candidate.typeLine || '').toLocaleLowerCase();
      if (sacrificeType === 'creature') return type.includes('creature');
      if (sacrificeType === 'artifact') return type.includes('artifact');
      if (sacrificeType === 'enchantment') return type.includes('enchantment');
      if (sacrificeType === 'land') return type.includes('land');
      return true;
    });

    candidates.sort((a, b) => botCardRoleScore(a) - botCardRoleScore(b));
    if (!candidates.length) return { ok: false, reason: `No ${sacrificeType} is available to sacrifice.` };
    choices.sacrificeId = candidates[0].instanceId;
  }

  if (/\bdiscard a card\b/i.test(costText)) {
    const hand = [...(state.players?.[bot.playerId]?.zones?.hand || [])]
      .sort((a, b) => botCardRoleScore(a) - botCardRoleScore(b));
    if (!hand.length) return { ok: false, reason: 'No card is available to discard.' };
    choices.discardId = hand[0].instanceId;
  }

  return { ok: true, choices, ability };
}

function botExecuteRealActivatedAbility(step, scoredResult = step) {
  const state = getState();
  const found = findCard(step.cardId, state);
  if (!found || found.zone !== 'battlefield') return false;

  const plan = botActivatedAbilityCostChoices(found.card, Number(step.abilityIndex || 0), state);
  if (!plan.ok) {
    bot.lastAction = `Held ${found.card.name}`;
    bot.lastReason = plan.reason || 'The activated ability cost could not be paid.';
    return false;
  }

  const result = activateBattlefieldAbility(
    found.card.instanceId,
    Number(step.abilityIndex || 0),
    plan.choices
  );

  if (!result?.ok) {
    bot.lastAction = `Could not activate ${found.card.name}`;
    bot.lastReason = result?.message || 'Activated ability could not be used.';
    return false;
  }

  // The actual Forge game engine has now paid the costs. This matters for
  // fetch lands: Terramorphic Expanse is tapped/sacrificed as a COST, then the
  // queued library search is resolved from the bot's real hidden library.
  const resolvedCount = botResolveOwnSupportedPendingEffects(12, {
    sourceCardId: found.card.instanceId,
  });

  const remaining = (getState().pendingTriggers || []).filter((effect) =>
    effect.controllerId === bot.playerId
    && effect.sourceCardId === found.card.instanceId
  );

  bot.lastAction = `Activated ${found.card.name}`;
  bot.lastReason = remaining.length
    ? `Paid the real activation cost, but Forge still needs rules support for: ${remaining[0].text}`
    : resolvedCount
      ? `${found.card.name}'s ability and its resulting effect resolved automatically.`
      : botActionReason(scoredResult);

  if (remaining.length) {
    bot.paused = true;
    toast(`${found.card.name}: activation cost was paid, but one instruction still needs rules support. Bot paused.`, true);
  }
...
function botExecuteTacticalStep(step, scoredResult = step) {
  ...
  if (step.type === 'activate-ability') return botExecuteRealActivatedAbility(step, scoredResult);
  const current = getState();
...
function botDeterministicFallbackAction(state, error = null) {
  const player = state.players?.[bot.playerId];
  if (!player) return false;
  const phase = PHASES[state.phaseIndex]?.id || '';

  // Main phase: play a land if possible using the real game engine.
  if (['main1', 'main2'].includes(phase)) {
    const land = (player.zones.hand || []).find((card) => isLand(card));
    const allowance = landPlayAllowance(state, bot.playerId);
    if (land && Number(player.landPlaysThisTurn || 0) < Number(allowance || 1)) {
      const result = moveCard(land.instanceId, bot.playerId, 'battlefield');
      if (result?.ok) {
        bot.lastAction = `Fallback play ${land.name}`;
        bot.lastReason = error
          ? `Tactical search overflowed, so Forge used a safe deterministic play instead: ${error.message}.`
          : 'Used a deterministic fallback play.';
        return true;
      }
    }
  }

  // Combat: if we have legal attackers, declare a conservative attack.
  if (phase === 'combat') {
    const attackers = (player.zones.battlefield || []).filter((card) =>
      isCreature(card)
      && !card.tapped
      && (!card.summoningSick || /haste/i.test(`${card.oracleText || ''} ${(card.manualKeywords || []).join(' ')}`))
    );

    if (attackers.length) {
      let declared = 0;
      for (const card of attackers.slice(0, 4)) {
        const result = toggleAttack(card.instanceId);
        if (result?.ok) declared += 1;
      }
      if (declared) {
        bot.pendingCombat = { attackerId: bot.playerId, label: `Fallback attack with ${declared}` };
        bot.lastAction = 'Fallback combat';
        bot.lastReason = 'Tactical search failed, so Forge used a simple legal combat line rather than pausing.';
        return true;
      }
    }
  }
  ...
  if (phaseResult?.ok) {
    bot.actionsThisPhase = 0;
    bot.lastAction = 'Fallback phase advance';
    bot.lastReason = error
      ? `Tactical search hit ${error.message}; advanced safely instead of pausing or recursing.`
      : 'Advanced safely.';
    return true;
  }
...
async function runBotTurnDecision(state) {
  ...
  const phaseKey = `${turnNumber}:${Number(state.phaseIndex || 0)}`;

  if (bot.lastTurnNumber !== turnNumber) {
    bot.lastTurnNumber = turnNumber;
    bot.actionsThisTurn = 0;
    bot.actionsThisPhase = 0;
    bot.lastPhaseKey = phaseKey;
  } else if (bot.lastPhaseKey !== phaseKey) {
    bot.lastPhaseKey = phaseKey;
    bot.actionsThisPhase = 0;
  }

  // The old counter was turn-wide, so once it hit 18 the bot would "safety
  // pass" every remaining phase. Use separate phase/turn guards instead.
  if (bot.actionsThisPhase >= 8 || bot.actionsThisTurn >= 32) {
    bot.lastAction = 'Safety phase advance';
    bot.lastReason = bot.actionsThisPhase >= 8
      ? 'Phase action limit reached; advanced once and reset the phase counter.'
      : 'Turn action limit reached; advanced safely to avoid an autoplay loop.';
    bot.actionsThisPhase = 0;
    const result = nextPhase();
  ...
  try {
    ...
    result = await analyzePositionAsync(snapshot, bot.playerId, config.rollouts, {
      maxDurationMs: config.maxDurationMs,
      shouldCancel: () => !botIsActiveSolo() || bot.paused || bot.pendingSpell || bot.pendingCombat,
    });
    bot.analysisStackFailures = 0;
  } catch (error) {
    bot.thinking = false;
    const isStackOverflow = error instanceof RangeError || /maximum call stack|stack size exceeded/i.test(String(error?.message || ''));

    if (isStackOverflow) {
      bot.analysisStackFailures += 1;
      return botDeterministicFallbackAction(state, error);
    }
  ...
  if (!result?.results?.length || !botIsActiveSolo() || bot.paused) {
    return botDeterministicFallbackAction(state);
  }

  const chosen = botChooseResult(result.results);
  const step = executableStep(chosen);

  if (!step) return botDeterministicFallbackAction(state);

  const acted = botExecuteTacticalStep(step, chosen);

  if (acted) {
    bot.actionsThisTurn += 1;
    bot.actionsThisPhase += 1;
  }
...
function botResolveOwnSupportedPendingEffects(maxSteps = 10, options = {}) {
  ...
  for (let step = 0; step < Math.max(1, Number(maxSteps || 10)); step += 1) {
    ...
    if (smart.kind === 'fight-creatures') {
      const preferred = options.sourceCardId === effect.sourceCardId ? options.targetPlan : null;
      let ownId = preferred?.ownTargetId || null;
      let opponentId = preferred?.opponentTargetId || null;

      if (!ownId && smart.ownCards?.length) ownId = smart.ownCards[0].instanceId;
      if (!opponentId && smart.opponentCards?.length) opponentId = smart.opponentCards[0].instanceId;

      result = resolveSmartFightEffect(effect.id, ownId, opponentId);
    } else if (smart.kind === 'search-library') {
      ...
      if (!chooseCount) {
        ...
        const chosenCards = ordered.slice(0, chooseCount);
      result = resolveSmartLibraryEffect(effect.id, chosenCards.map((card) => card.instanceId));
      if (result?.ok && chosenCards.length) {
        bot.lastReason = `${effect.sourceName}: found ${chosenCards.map((card) => card.name).join(', ')}${smart.destination === 'battlefield' ? ` and put ${chooseCount === 1 ? 'it' : 'them'} onto the battlefield${smart.tapped ? ' tapped' : ''}` : ''}${smart.shuffle ? ', then shuffled' : ''}.`;
      }
      }
...
function botAdvanceStackPriority() {
  ...
  if (bot.pendingSpell && top.instanceId === bot.pendingSpell.cardId) {
    const liveCard = top;
    const pending = deepClone(bot.pendingSpell);

    // Tactical analysis decides WHAT the bot should cast.
    // Commander Forge rules decide WHAT THE SPELL ACTUALLY DOES.
    ...
    botResolveOwnSupportedPendingEffects(12, {
      sourceCardId: liveCard.instanceId,
      targetPlan: pending.targetPlan || null,
    });

    const unresolved = (getState().pendingTriggers || []).filter((effect) =>
      effect.controllerId === bot.playerId
      && effect.sourceCardId === liveCard.instanceId
    );

    if (unresolved.length) {
      bot.paused = true;
      bot.lastReason = `Paused because Forge does not yet have a deterministic resolver for: ${unresolved[0].text}`;
      toast(`${liveCard.name}: one instruction still needs rules support. Bot paused instead of pretending it resolved.`, true);
    } else {
      bot.lastReason = pending.targetPlan?.targetNames?.length
        ? `Resolved automatically using targets ${pending.targetPlan.targetNames.join(' → ')}.`
        : 'Resolved through Commander Forge rules rather than a tactical approximation.';
      botSchedule(botDelay());
    }
...
async function runBotLoop() {
  ...
  try {
    ...
    if (acted && !bot.pendingSpell && !bot.pendingCombat && !bot.paused) {
      botSchedule(botDelay());
    } else {
      render({ force: true });
    }
  } finally {
    bot.loopRunning = false;
  }
}
...
function renderLibraryEffectPicker(state) {
  ...
  const destinationLabel = uiLibraryDestinationLabel(smart.destination);
...
function renderCompactOpponentMat(state, playerId) {
  ...
  return `<section class="compact-opponent-mat ${active ? 'active-player' : ''} ${player.lost ? 'lost-player' : ''}" data-player-mat="${playerId}">
    <header class="compact-opponent-header"><div><span class="compact-player-seat">P${String(playerId).replace(/^p/,'')}</span><strong>${escapeHtml(player.name)}</strong>${state.monarchPlayerId === playerId ? '<span class="monarch-badge">👑</span>' : ''}${active ? '<span class="active-turn-badge">ACTIVE</span>' : ''}</div><div class="compact-life-row"><span>♥ ${Number(player.life||0)}</span><span>☠ ${Number(player.poison||0)}</span><span>✋ ${handCount}</span></div></header>
...
function renderPlayerStatus(state, playerId) {
  ...
  return `
    <div class="player-status ${active ? 'active' : ''}">
      <div class="player-name-row"><span class="player-name">${escapeHtml(player.name)}</span>${state.monarchPlayerId === playerId ? '<span class="monarch-badge" title="Monarch">👑 MONARCH</span>' : ''}${active ? '<span class="active-dot" title="Active player"></span>' : ''}</div>
      <div class="trackers">
...
function uiLibraryDestinationLabel(destination) {
  if (destination === 'library-top') return 'top of library';
  try {
    if (typeof smartLibraryDestinationLabel === 'function') {
      return smartLibraryDestinationLabel(destination);
    }
  } catch {}
  const labels = {
    hand: 'hand',
  ...
  return labels[destination] || String(destination || 'destination');
}
...
function renderSmartEffectActions(effect, state) {
  ...
  if (smart.kind === 'search-library') {
    const destination = uiLibraryDestinationLabel(smart.destination);
...
function renderOracleCompilerDiagnostic(card) {
  const compiler = globalThis.CommanderForgeOracleCompilerV7;
  if (!compiler || !card?.oracleText) return '';

  let program = null;
  let statics = null;

  try {
    program = compiler.compileProgram?.(card.oracleText, { cardName: card.name }) || null;
    statics = compiler.compileStaticAbilities?.(card.oracleText, { cardName: card.name }) || null;
  } catch {
    return '';
  }

  const attackCosts = (statics?.staticAbilities || []).filter((item) => item.type === 'ATTACK_COST');

  if ((!program?.steps?.length) && !(statics?.staticAbilities || []).length) return '';

  const recognized = program?.steps?.filter((step) => step.type !== 'UNSUPPORTED').length || 0;
  const total = program?.steps?.length || 0;
  const percent = total ? Math.round(Number(program.coverage || 0) * 100) : 0;
  const unsupported = program?.unsupported?.[0]?.text || '';

  const restrictionLines = (statics?.staticAbilities || [])
    .filter((item) => ['CAST_PROHIBITION', 'BLOCK_PROHIBITION', 'CAST_LIMIT'].includes(item.type))
    .map((item) => {
      if (item.type === 'CAST_LIMIT') {
        return `<span class="compiler-ready">Static rule: players are limited to ${Number(item.limitPerTurn || 1)} spell each turn.</span>`;
      }

      const subject = item.appliesTo === 'OPPONENTS' ? 'opponents' : item.appliesTo === 'ALL_PLAYERS' ? 'players' : 'controller';
      const parity = item.predicate?.property === 'MANA_VALUE_PARITY'
        ? `${String(item.predicate.parity || '').toLocaleLowerCase()} mana value`
        : item.predicate?.property === 'MANA_VALUE_COMPARE'
          ? `restricted mana value`
          : 'matching cards';

      return item.type === 'CAST_PROHIBITION'
        ? `<span class="compiler-ready">Static rule: ${escapeHtml(subject)} can’t cast ${escapeHtml(parity)} spells.</span>`
        : `<span class="compiler-ready">Static rule: ${escapeHtml(subject)} can’t block with ${escapeHtml(parity)} creatures.</span>`;
    }).join('');

  const staticLines = attackCosts.map((item) => {
    const dynamic = item.dynamicAmount === 'ENCHANTMENTS_YOU_CONTROL'
      ? 'X = enchantments you control'
      : '';
    const condition = item.sourceMustBeUntapped ? 'while this permanent is untapped' : '';
    return `<span class="compiler-ready">Static combat rule: attackers must pay ${escapeHtml(item.perAttackerCost)} each${dynamic ? ` · ${escapeHtml(dynamic)}` : ''}${condition ? ` · ${escapeHtml(condition)}` : ''}</span>`;
  }).join('') + restrictionLines;

  const programLine = total
    ? `<span>${recognized}/${total} instruction${total === 1 ? '' : 's'} recognized · ${percent}% ordered-effect coverage</span>`
    : '';

  const programStatus = program?.authoritative
    ? '<span class="compiler-ready">Ordered auto-resolution ready</span>'
    : unsupported
      ? `<span class="compiler-warning">Ordered effect needs adapter: ${escapeHtml(unsupported)}</span>`
      : '';

  return `<div class="compiler-status ${(program?.authoritative || (statics?.staticAbilities || []).length) ? 'ready' : 'partial'}">
    <strong>Oracle Compiler ${escapeHtml(compiler.VERSION || program?.compilerVersion || '')}</strong>
    ${programLine}
    ${staticLines}
    ${programStatus}
  </div>`;
...
function pendingEffectUseForCard(selected, state) {
  if (!selected?.card) return null;

  for (const effect of state.pendingTriggers || []) {
    const smart = smartPendingEffectState(effect, state);
    if (!multiplayerCanResolvePendingEffect(effect, smart, state)) continue;

    if (
      selected.zone === 'library'
      && smart.kind === 'search-library'
      && Number(smart.maxCount || 1) === 1
      && smart.cards?.some((card) => card.instanceId === selected.card.instanceId)
    ) {
      return {
        effectId: effect.id,
        kind: 'library',
        sourceName: effect.sourceName,
        destination: uiLibraryDestinationLabel(smart.destination),
      };
    }

    if (
      selected.zone === 'graveyard'
      && smart.kind === 'graveyard-move'
      && Number(smart.maxCount || 1) === 1
      && smart.cards?.some((card) => card.instanceId === selected.card.instanceId)
    ) {
      return {
...
function renderInspector(state) {
  ...
  const pendingUse = pendingEffectUseForCard(selected, state);
  return `
    ...
    <div class="inspector-section"><h3>Quick actions</h3><div class="action-grid">
      ${pendingUse ? `<button class="btn primary wide" data-action="use-card-for-pending-effect" data-effect-id="${pendingUse.effectId}" data-card-id="${card.instanceId}" data-effect-kind="${pendingUse.kind}">✓ Use for ${escapeHtml(pendingUse.sourceName)} → ${escapeHtml(pendingUse.destination)}</button>` : ''}
      ...
      ${selected.zone === 'battlefield' ? `<button class="btn danger" data-action="sacrifice-permanent" data-card-id="${card.instanceId}" title="Use this when a spell, ability, or game action tells you to sacrifice this permanent.">☠ Sacrifice</button>` : ''}
...
function renderAttackTargetPicker(state) {
  ...
  return `<div class="modal-backdrop"><section class="modal small-modal attack-target-modal"><header class="modal-header"><div><h2>Who is ${escapeHtml(found.card.name)} attacking?</h2><p class="small muted">In multiplayer Commander, each attacker can attack a different opponent. Attack taxes are checked before the attacker is declared.</p></div><button class="icon-btn" data-action="close-attack-target">×</button></header><div class="modal-body"><div class="attack-target-grid">${choices.map((id)=>{
    const tax=attackTaxPlan(state,found.card.controller||found.playerId,id,1,{useSources:state.settings.manaMode!=='manual',maxNodes:4000});
    const taxLine=tax.required
      ? `<small class="attack-tax-note ${tax.ok ? '' : 'unpayable'}">⚖ Attack tax ${escapeHtml(tax.manaCost || 'manual')} · ${escapeHtml((tax.sourceNames || []).join(', '))}${tax.ok ? '' : ' · cannot currently pay'}</small>`
      : '';
    return `<button class="attack-target-choice" data-action="choose-attack-target" data-card-id="${cardId}" data-target-player-id="${id}"><span>P${String(id).replace(/^p/,'')}</span><strong>${escapeHtml(state.players[id].name)}</strong><small>${Number(state.players[id].life||0)} life · ${Number(state.players[id].poison||0)} poison</small>${taxLine}</button>`;
...
function renderSettingsModal(state) {
  const v6 = window.CommanderForgeEngine6?.getStatus?.() || { state: 'starting', version: '6.13.0' };
  const engineStatus = v6.state === 'ready'
    ? `<div class="validation ok"><strong>Engine 6 ready.</strong> ${escapeHtml(v6.version || '6.13.0')} · Browser Web Worker${v6.lastSyncAt ? ' · table mirrored' : ''}${v6.lastCompile?.cards != null ? ` · ${v6.lastCompile.cards} card definitions analyzed${v6.lastCompile.unsupportedClauses ? ` · ${v6.lastCompile.unsupportedClauses} unsupported Oracle clause(s) kept for fallback` : ''}` : ''}</div>`
    : v6.state === 'error'
...
app.addEventListener('click', async (event) => {
  ...
  try {
    ...
    if (action === 'release-reserved-seat') handleResult(releaseReservedSeat(button.dataset.playerId));
    ...
    if (action === 'use-card-for-pending-effect') {
      const effectId = button.dataset.effectId;
      const cardId = button.dataset.cardId;
      const kind = button.dataset.effectKind;

      const result = kind === 'library'
        ? resolveSmartLibraryEffect(effectId, [cardId])
        : resolveSmartPendingEffect(effectId, cardId);
    ...
    if (action === 'sacrifice-permanent') handleResult(sacrificePermanent(button.dataset.cardId));
    ...
    if (action === 'resolve-stack') {
      const result = resolveStackTop();
      handleResult(result);
      if (result?.ok) maybeOpenNewestInteractiveEffect();
    }
...
function maybeOpenNewestInteractiveEffect() {
  const state = getState();
  const localId = isOnlineMultiplayer() && multiplayer.localPlayerId
    ? multiplayer.localPlayerId
    : 'p1';

  const candidates = [...(state.pendingTriggers || [])].filter((effect) => {
    const smart = smartPendingEffectState(effect, state);
    return multiplayerCanResolvePendingEffect(effect, smart, state)
      && (smart.kind === 'search-library' || smart.kind === 'graveyard-move');
  });
  ...
  if (smart.kind === 'search-library' && smart.playerId === localId) {
    ui.libraryEffectPicker = { effectId: effect.id, selected: [], search: '' };
    render({ force: true });
    return true;
  }
  ...
  if (smart.kind === 'graveyard-move') {
    ...
    ui.inspectorMode = 'card';
    render({ force: true });
    toast(`${effect.sourceName}: choose the matching graveyard card from the pending effect.`);
    return true;
  }
