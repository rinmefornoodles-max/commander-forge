...
vm.runInContext(fs.readFileSync(path.join(root, 'commander-forge-blocking-hotfix-6.14.1.js'), 'utf8'), ctx, { filename: 'blocking-hotfix.js' });
...
test('manual blocker assignment does not throw and records the block',()=>{const state=setup(2);state.players.p1.zones.battlefield=[creature('a','p1')];state.players.p2.zones.battlefield=[creature('b','p2')];api.state.setState(state,{persist:false});assert.equal(api.game.toggleAttack('a','p2').ok,true);const result=api.game.assignBlocker('b','a');assert.equal(result.ok,true);const next=api.state.getState();assert.equal(next.players.p2.zones.battlefield[0].blocking,'a');assert.deepEqual([...next.players.p1.zones.battlefield[0].blockedBy],['b']);});
test('manual blocker assignment rejects an illegal flying block',()=>{const state=setup(2);const flyer=creature('a','p1','Wind Drake','Flying');flyer.manualKeywords=['Flying'];state.players.p1.zones.battlefield=[flyer];state.players.p2.zones.battlefield=[creature('b','p2')];api.state.setState(state,{persist:false});assert.equal(api.game.toggleAttack('a','p2').ok,true);const result=api.game.assignBlocker('b','a');assert.equal(result.ok,false);assert.match(result.message,/cannot legally block/i);});
