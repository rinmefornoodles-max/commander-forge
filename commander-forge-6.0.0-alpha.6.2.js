...
const MULTIPLAYER_APP_VERSION = '6.0.0-alpha.6.2-mp6';
const multiplayer = {
  mode: 'solo',
  role: null,
  localPlayerId: null,
...
function renderStartingRollModal(state) {
  ...
  return `<div class="modal-backdrop starting-roll-backdrop"><section class="modal starting-roll-modal" role="dialog" aria-modal="true" aria-labelledby="starting-roll-title"><header class="starting-roll-header"><div><div class="mulligan-kicker">WHO GOES FIRST?</div><h2 id="starting-roll-title">Throw the D20</h2><p>${escapeHtml(turnPrompt || roll.message || 'Highest roll goes first.')}</p></div><span class="d20-round">Round ${Number(roll.round || 1)}</span></header><div class="starting-roll-body"><div class="d20-results-grid">${results}</div><div class="d20-arena-wrap"><canvas id="forge-d20-canvas" data-can-roll="${canRoll ? 'true' : 'false'}"></canvas>${canRoll ? '<div class="d20-instruction"><strong>Click and hold the die, drag, then release to throw.</strong><span>Throw speed is capped so the die stays on the table. It has momentum, friction, and bounces off the arena edges.</span></div>' : waitingText}</div></div></section></div>`;
}
...
function renderSettingsModal(state) {
  const v6 = window.CommanderForgeEngine6?.getStatus?.() || { state: 'starting', version: '6.0.0-alpha.6.2' };
  const engineStatus = v6.state === 'ready'
    ? `<div class="validation ok"><strong>Engine 6 ready.</strong> ${escapeHtml(v6.version || '6.0.0-alpha.6.2')} · Browser Web Worker${v6.lastSyncAt ? ' · table mirrored' : ''}${v6.lastCompile?.cards != null ? ` · ${v6.lastCompile.cards} card definitions analyzed${v6.lastCompile.unsupportedClauses ? ` · ${v6.lastCompile.unsupportedClauses} unsupported Oracle clause(s) kept for fallback` : ''}` : ''}</div>`
    : v6.state === 'error'
