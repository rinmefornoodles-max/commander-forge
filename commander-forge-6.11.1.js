...
// ---- main.js ----
globalThis.CommanderForgeBuildVersion = '6.11.1';
(() => {
...
const { commanderCandidates, landPlayAllowance, maximumHandSize, moveLegality, recognizedEffects, validateDeck } = __modules["./rules.js"];
...
const MULTIPLAYER_APP_VERSION = '6.11.1-mp6';
...
function renderOracleCompilerDiagnostic(card) {
  const compiler = globalThis.CommanderForgeOracleCompilerV7;
  if (!compiler?.compileProgram || !card?.oracleText) return '';

  let program = null;
  try {
    program = compiler.compileProgram(card.oracleText, { cardName: card.name });
  } catch {
    return '';
  }

  if (!program?.steps?.length) return '';

  const recognized = program.steps.filter((step) => step.type !== 'UNSUPPORTED').length;
  const total = program.steps.length;
  const percent = Math.round(Number(program.coverage || 0) * 100);
  const unsupported = program.unsupported?.[0]?.text || '';

  return `<div class="compiler-status ${program.authoritative ? 'ready' : 'partial'}">
    <strong>Oracle Compiler ${escapeHtml(program.compilerVersion || '')}</strong>
    <span>${recognized}/${total} instruction${total === 1 ? '' : 's'} recognized · ${percent}% coverage</span>
    ${program.authoritative
      ? '<span class="compiler-ready">Ordered auto-resolution ready</span>'
      : unsupported
        ? `<span class="compiler-warning">Needs adapter: ${escapeHtml(unsupported)}</span>`
        : '<span class="compiler-warning">Recognized, but not yet live-authoritative.</span>'}
  </div>`;
}
...
function renderSettingsModal(state) {
  const v6 = window.CommanderForgeEngine6?.getStatus?.() || { state: 'starting', version: '6.11.1' };
  const engineStatus = v6.state === 'ready'
    ? `<div class="validation ok"><strong>Engine 6 ready.</strong> ${escapeHtml(v6.version || '6.11.1')} · Browser Web Worker${v6.lastSyncAt ? ' · table mirrored' : ''}${v6.lastCompile?.cards != null ? ` · ${v6.lastCompile.cards} card definitions analyzed${v6.lastCompile.unsupportedClauses ? ` · ${v6.lastCompile.unsupportedClauses} unsupported Oracle clause(s) kept for fallback` : ''}` : ''}</div>`
    : v6.state === 'error'
